...
ERROR_CODE_CSV_PATH = os.path.dirname(os.path.realpath(__file__)) + "/acceptance_criteria.csv"

DEEPSIGHT_SCRIPT = os.path.dirname(os.path.realpath(__file__)) + "/deepsight2.sh"

OUTPUT_DIRECTORY = r"/raid/mpsych/OMAMA/DEEPSIGHT/deepsight_out/"

CACHE_PATH = os.path.dirname(os.path.realpath(__file__)) + "/predictions_cache.json"
...
class DeepSight(Algorithms):
    ...
    def run(
        ...
        pred_cache_path = pred_cache_path or CACHE_PATH

        deepsight_script_path = deepsight_script_path or DEEPSIGHT_SCRIPT

        # set the input nad output paths
        output_dir = output_dir or OUTPUT_DIRECTORY
        # make sure the output directory and any parent directories exist and if
        # not create them
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        caselist_file_name = caselist_file_name or CASELIST_FILE_NAME

        ignore_checks = ignore_checks or IGNORE_CHECKS
