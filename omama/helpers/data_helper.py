...
class DataHelper(object):
    ...
    # --------------------------------------------------------------------------
    @staticmethod
    def _add_bounding_box(image, pred, sop_uid=None):
        ...
        # set the linewiodth of the bounding box between 2 and 10 based on size
        # of image
        ...
        if sop_uid:
            ...
            # pred is the bounding box coordinates
            bb = pred
            bb = [int(p) for p in bb]

        print(f"bb: {bb}")

        img_copy[bb[1] : bb[3], bb[0] : bb[0] + linewidth] = 0
        img_copy[bb[1] : bb[3], bb[2] : bb[2] + linewidth] = 0
        img_copy[bb[1] : bb[1] + linewidth, bb[0] : bb[2]] = 0
        img_copy[bb[3] : bb[3] + linewidth, bb[0] : bb[2]] = 0
    ...
    @staticmethod
    def get3D(
        N=1, data_loader=None, cancer=None, randomize=False, config_num=3, timing=False
    ):
    ...
    def view(
        ...
        roi_coords : list
            (default is None) If specified, will add a bounding box to the image.
        slice_num : int
            (default is None)
        ...
        if prediction is not None:
            img, bb = DataHelper._add_bounding_box(img, prediction, sop_uid)
        if roi_coords is not None:
            img, bb = DataHelper._add_bounding_box(img, roi_coords)
        ...
        if (prediction is not None or roi_coords is not None) and bb is not None:
            plt.gca().add_patch(
                plt.Rectangle(
                    (bb[0], bb[1]),
                    bb[2] - bb[0],
