...
def worker(npz_file, input_dir, output_dir, target_size=(512, 512)):
...
def worker_with_progress(npz_file, input_dir, output_dir, target_size, lock, counter):
    worker(npz_file, input_dir, output_dir, target_size)
    with lock:
        counter.value += 1
...
def resize_and_save_images(
    input_dir, output_dir, target_size=(512, 512), num_processes=None
):
    ...
    with Pool(processes=num_processes) as pool:
        pool.starmap(
            ...
            [
                (npz_file, input_dir, output_dir, target_size, lock, counter)
...
if __name__ == "__main__":
    ...
    resize_and_save_images(input_dir, output_dir, target_size, num_processes)
    print("Done.")
