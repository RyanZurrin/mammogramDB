...
class KaggleSight:
    ...
    def run(
        ...
        while not KaggleSight._validate_unique_filename(output_dir, unique_filename):
            unique_filename = KaggleSight._generate_unique_filename()
        ...
        error_codes_dict = (
            KaggleSight._generate_error_codes_dict()
        ...
        if force_run is False:
            predictions, caselist_file, run_flag = KaggleSight._check_predictions_cache(
        ...
        if run_flag:
            ...
            print(f"Running KaggleSight on {run_size} cases, please be patient...")
                        ...
                        if json_result["_metadata"] is not None:
                            ...
                            for k, v in metadata_dict[target_id].items():
                                if metadata_dict["failed_checks"][k] is not None:
                                    id_to_sop_dict[int(k)] = metadata_dict[target_id][k]
            ...
            KaggleSight._update_predictions_cache(
                predictions, pred_cache_path, timing=timing
            )
        # create the errors.txt file
        KaggleSight._create_errors_txt(
            output_location, KaggleSight.sop_to_path_dict, predictions
        )
        ...
        if len(predictions) == 0:
            ...
            print("*" * 124)
            print(
                f"No predictions made, printing output file located at {output_location}deepsight_out.txt"
