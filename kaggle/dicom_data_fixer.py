...
# parser.add_argument('-i', '--inplace', help='Inplace',
#                     default=INPLACE, action='store_true')
# # parameters is a dictionary so use json loads to convert it to a dictionary from a string
# parser.add_argument('-p', '--parameters', help='Parameters to change',
#                     type=json.loads, default=JSON_PARAMETERS)
# args = parser.parse_args()


class DicomDataFixer:
    def __init__(self, dicom_dir, csv_path, output_dir=None, inplace=False, **kwargs):
    ...
    def get_age(self, dicom_data):
        ...
        age = self.csv_df.loc[self.csv_df["image_id"] == inst_num, "age"].iloc[0]
        age = age * 12
        return str(age) + "m"

    def get_veiw(self, dicom_data):
        inst_num = dicom_data.InstanceNumber
        view = self.csv_df.loc[self.csv_df["image_id"] == inst_num, "view"].iloc[0]
        return view

    def get_breast_implant_present(self, dicom_data):
        inst_num = dicom_data.InstanceNumber
        breast_implant_present = self.csv_df.loc[
            self.csv_df["image_id"] == inst_num, "implant"
        ].iloc[0]
    ...
    def generate_accession_number(self):
        ...
        """
        accession_number = "".join(random.choices(string.ascii_uppercase, k=12))
    ...
    def get_patient_orientations(self, dicom_data):
        ...
        laterality = self.csv_df.loc[
            self.csv_df["image_id"] == inst_num, "laterality"
        ].iloc[0]
        if laterality == "R":
    ...
    def fix_dicoms(self):
        ...
        for dicom_loc in self.image_paths:
            ...
            dicom_data.PatientOrientation = self.get_patient_orientations(dicom_data)
            ...
            for key, value in self.parameters.items():
                ...
                if self.inplace:
                    ...
                    dicom_data.save_as(
                        os.path.join(self.output_dir, os.path.basename(dicom_loc)),
        ...
        print(f"Finished fixing {count} dicom files in {time.time() - t0} " f"seconds.")


if __name__ == "__main__":
    # create the object
    fixer = DicomDataFixer(
        DICOM_DIR, CSV_PATH, inplace=True, output_dir=OUTPUT_DIR, **PARAMETERS
    )
