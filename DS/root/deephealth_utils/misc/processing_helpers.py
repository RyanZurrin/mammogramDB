from DS.root.deephealth_utils.data.input_processing import (
    target_and_rand_resize,
    flip_im,
    norm_array,
    orth_rotate,
    trim_image_background,
)
...
def transform_im_from_proc_info(im, proc_info):
    for f in [
        ...
        fname = f.__name__
        vals = (
            proc_info["proc_funcs"][fname]
            if "proc_funcs" in proc_info
            else proc_info[fname]
        )
