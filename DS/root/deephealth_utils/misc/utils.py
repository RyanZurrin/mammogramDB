...
def post_process_im(im, map_config, proc_info):
    ...
    if crop_info is not None:
        ...
        if im.shape != init_target_size:
            im = zoom_to_match_size(
                im, init_target_size, order=map_config["zoom_order"]
            )
        if np.sum(pad_width):
...
def preprocess_for_auc(y, y_hat, axis=-1):
    if y.ndim > 1:
        y = y.argmax(axis=axis)
    if y_hat.ndim > 1:
        y_hat = y_hat[:, 1]  # assuming the second column is the probability of the positive class
    return y, y_hat
