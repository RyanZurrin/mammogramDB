...
def scrub_wrapper(file_in, file_out, shell=False, tmp_file_path=None):
    ...
    subprocess.call(
        ["perl", path_to_deid + "/addHeader.pl", file_in, file_in_has_header],
        shell=shell,
    )
    subprocess.call(
        [
            "perl",
            path_to_deid + "/deid.pl",
            file_in_has_header,
            file_out,
            path_to_deid + "/deid-output.config",
            path_to_deid,
            "NO COMMENTARY",
        ],
        shell=shell,
    ...
    subprocess.call(
        ["perl", path_to_deid + "/scrubFormatter.pl", file_out], shell=shell
    )
