...
from DS.root.deephealth_utils.data.format_helpers import (
    ...
    format_dicom_tag_str,
    map_manufacturer,
    verbose_position_to_code,
)
from DS.root.deephealth_utils.data.dicom_utils import determine_if_hologic_sco
from DS.root.deephealth_utils.misc.gen_helpers import is_str_type
...
def window_im(x, center, width, ymin=0.0, ymax=65535.0, return_type="uint16"):
...
def window_im_sigmoid(x, center, width, ymin=0.0, ymax=65535.0, return_type="uint16"):
    ...
    assert type(center) in [int, float], '"center" is not numeric type'
    assert type(width) in [int, float], '"width" is not numeric type'
    y = ymax / (1.0 + np.exp(-4.0 * (x.astype(float) - center) / width))
    if "int" in return_type:
        y = np.round(y)
...
def nested_index(data, tag, raise_attribute_error=True):
    ...
    def _getattr(
        data, tag
    ):  # Need to already make sure tag is either list of codes or string
        if _isdcmcode(tag) or isinstance(
            tag, int
        ):  # Account for either receiving a tuple of str ints or just one int
...
def dcm_to_array(ds, orig_file_path=None):
    ...
    if is_str_type(ds):
        assert orig_file_path is None or ds == orig_file_path, (
            "File path specified by ds ("
            + ds
            + ") doesn't match orig_file_path ("
            + str(orig_file_path)
            + ")"
        )
        orig_file_path = ds
    ...
    if determine_if_hologic_sco(ds):
        assert (
            orig_file_path is not None
        ), "Need to specify file path when extracting Hologic SC pixels"
...
def dcm_to_preprocessed_array(
    ...
    """
    assert ds is not None or (
        x is not None and attribute_dict is not None
    ...
    if get_value("WindowCenterWidthExplanation") in ["", None]:
        ...
        explanations = list(
            get_value("WindowCenterWidthExplanation")
    ...
    if manu.lower() == "hologic":
        ...
        if standardize_window_params:
            if get_value("HighBit") == 11:
                center = min(
                    center, 3060
                )  # one standard deviation above median in AST data
    ...
    if slice_num is not None:
        assert x.ndim == 3, "Trying to get slice_num from an array of dimension " + str(
            x.ndim
        )
        assert slice_num >= 0, "slice_num must be >=0"
        assert slice_num < x.shape[0], (
            "array has "
            + str(x.shape[0])
            + " slices and desired slice_num is "
            + str(slice_num)
        )
        x = x[slice_num]
...
def evaluate_alt_tag(row, ds):
    ...
    if (
        isinstance(alt_tag[0], list)
        and len(alt_tag[0][0]) == 2
        and Tag(alt_tag[0][0]).is_private
    ):
...
def get_dcm_attribute(
    ds, spec_id, alt_tags, tag, convert_to_string=False, raise_none_error=False
):
    """
    get DICOM attribute
    Args:
    ...
    """
    is_str_tag_type = (
        type(tag) in [str, unicode] if sys.version[0] == "2" else isinstance(tag, str)
    )
    if is_str_tag_type:
    ...
    if value is None:
        if is_str_tag_type:
            sub_df = alt_tags[
                (alt_tags["spec_id"] == spec_id) & (alt_tags["field"] == tag)
            ]
    ...
    for alt_tag in ALT_TAGS.tag.values:
        ...
        if isinstance(alt_tag[0], list) and Tag(alt_tag[0][0]).is_private:
            if len(alt_tag) != 3 or len(alt_tag[0][0]) != 2:
                return (
                    False,
                    "Tag specified needs to contain [attr_tag, tag_to_compare, expected_val]",
                )
...
def dh_dcmread(file_or_folder_name, stop_before_pixels=False, external_metadata=None):
    ...
    if external_metadata is None:
        ...
        ds = DHDataset()
        assert isinstance(
            external_metadata, pd.Series
        ), f"pandas Series expected. Got: {external_metadata}"
...
def get_study_metadata(
    ...
    for field in dicom_fields:
        ...
        if (
            df_available
            and field in metadata_df.columns
            and not field in special_fields
        ):
            df = metadata_df[
                (metadata_df[field].notnull()) & (metadata_df[field] != "")
            ]
            if df.empty:
            ...
            for i in range(len(file_list)):
                if cached_files[i] is None:
                    try:
                        cached_files[i] = dh_dcmread(
                            file_list[i], stop_before_pixels=True
                        )
                    except:
                        logging.getLogger().exception(
                            "File ({}) could not be read".format(file_list[i])
                        )
                        continue
                ds = cached_files[i]
