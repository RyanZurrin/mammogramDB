...
class DicomTypeMap(object):
    ...
    def get_type_row(row):
        if isinstance(row, pd.core.frame.DataFrame):
            assert len(row) == 1, "Numbers of rows is {} but needs to be 1".format(
                len(row)
            )
        ...
        if hasattr(pixel_array_shape, "__len__"):
            if len(pixel_array_shape) == 3:
                ...
                raise ValueError(
                    "Pixel array shape is {} but should be 2 or 3".format(
                        len(pixel_array_shape)
                    )
                )
        else:
            if number_of_frames in ["", 1, np.nan]:
                ...
                raise ValueError(
                    "Number of frames is unrecognized type, {}".format(number_of_frames)
                )
