...
def standard_study_description(study_desc, default_modality=None):
    ...
    if default_modality is not None:
        if "screen" in study_desc:
            assert default_modality in ["Mammogram", "Tomosynthesis"], (
                "Unrecognized modality: " + default_modality
            )
    ...
    if keywords["Mammogram"]:
        mamm_mod = (
            "Tomosynthesis"
            if ("3d" in study_desc or "tomo" in study_desc or "dbt" in study_desc)
            else "Mammogram"
        )
