...
class ImageInputProcessor(object):
    def __init__(
        self,
        resize_targets=[None],
        rand_resize_ranges=[None],
        flip_params=["rand"],
        rot_params=[None],
        norm_factor="auto",
        trim_background=False,
        is_contra_pairs=False,
        convert_to_rgb=False,
        imagenet_preprocess=False,
        divisible_factor=None,
        use_skimage=False,
    ):
        verify_init(
            resize_targets,
        ...
        self.process_funcs = [
            [target_and_rand_resize, flip_im, norm_array, orth_rotate]
            for _ in range(self.n_scales)
        ]
        self.process_params = [
            [
                {
                    "resize_target": resize_targets[i],
                    "rand_range": rand_resize_ranges[i],
                    "use_skimage": use_skimage,
                },
                {"flip_param": flip_params[i]},
        ...
        if trim_background:
            for i in range(self.n_scales):
                self.process_funcs[i].insert(
                    2, trim_image_background
                )  # put it before norm_array in case that messes up with background calculation
        ...
        if divisible_factor is not None:
            for i in range(self.n_scales):
                ...
                self.process_params[i].insert(
                    -2, {"divisible_factor": divisible_factor}
                )

    def create_input(
        self, im_paths, return_extra_info=False, rotate_180=False, view=None
    ):
        ...
        for im_n, im_path in enumerate(im_paths):
            # Read Images
            if (not type(im_path) is np.ndarray) and (
                not type(im_path) is imageio.core.util.Array
            ):
        ...
        if len(orig_ims) > 1:
            ...
            for i in range(1, len(orig_ims)):
                if orig_ims[i].shape[0] != target_h:
                    ...
                    orig_ims[i] = transform.resize(
                        orig_ims[i],
                        (target_h, int(np.round(orig_ims[i].shape[1] * z))),
                        preserve_range=True,
                    )
        ...
        for i in range(self.n_scales):
            ims = process_im(
                orig_ims,
                process_funcs=self.process_funcs[i],
                process_params=self.process_params[i],
                    ...
                    if nh > 0 or nw > 0:
                        ...
                        if nw:
                            ...
                            if mean_l > mean_r:
                                ...
                                n_pad[1][0] = nw
                        im = np.pad(
                            im, n_pad, mode="constant", constant_values=np.min(im)
                        )
                        ims[im_i] = im
            ...
            if n_ims == 1:
                ...
                if self.is_contra_pairs:
                    if n_ims == 2:
                        ...
                        inputs[i + self.n_scales] = np.concatenate(
                            ims[n_ims / 2 :], self.concat_axis
                        )
                    continue
                else:
        ...
        if self.imagenet_preprocess:
            if (
                isinstance(self.imagenet_preprocess, (str, unicode))
                and self.imagenet_preprocess == "-1_1"
            ):
                for k in range(len(inputs)):
                    inputs[k] -= 0.5
                    inputs[k] *= 2
        ...
        if len(list_param) != len(resize_targets):
            ...
            if list_names[i] == "rot_params":
                if len(rot_params) > len(resize_targets):
                    ...
                    rot_params += [rot_params[-1]] * (
                        len(resize_targets) - len(rot_params)
                    )
    ...
    for i in range(len(resize_targets)):
        ...
        if type(rt) in [list, tuple]:
            if len(rt) < 2:
                ...
                message = (
                    "resize_targets[i] got length greater than 3 at index {}".format(i)
                )
                raise ValueError(message)
...
def target_and_rand_resize(
    im, resize_target=None, rand_range=None, return_extra_info=False, use_skimage=False
):
...
def trim_image_background(
            ...
            if np.max(im) <= background_lvl:
                ...
                for a in [1, 0]:
                    ...
                    if a == 0:
                        ...
                        if len(idx_back):
                            if (
                                idx_back[0] >= len(vals) - 1 - idx_back[-1]
                            ):  # breast on left side of image
                                i0 = 0
                                i1 = idx_back[0]
                            else:
                                i0 = idx_back[-1]
    ...
    if crops is not None:
        if (
            divisible_factor is not None
        ):  # make image size divisible by divisible_factor
            for axis in [0, 1]:
                ...
                if delta:
                    ...
                    to_pad[other_axis] = min(
                        delta - to_pad[min_axis], avail[other_axis]
                    )
...
def make_size_divisible(ims, divisible_factor, pad_value=0, return_extra_info=False):
    ...
    if np.sum(to_pad):
        ...
        for i in range(len(ims)):
            ims[i] = np.pad(
                ims[i], pad_width, mode="constant", constant_values=pad_value
            )
