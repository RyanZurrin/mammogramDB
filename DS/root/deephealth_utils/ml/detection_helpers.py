...
from DS.root.deephealth_utils.misc.processing_helpers import transform_im_from_proc_info
...
def box_coords_to_original(box_coords, proc_info):
    ...
    for b_num, box in enumerate(box_coords):
        ...
        if crop_info is not None:
            ...
            if np.sum(pad_width):
                init_target_size = (
                    crop_info[0][0][1] - crop_info[0][0][0],
                    crop_info[0][1][1] - crop_info[0][1][0],
                )
            else:
            ...
            if c_im.shape != init_target_size:
                v = [
                    float(init_target_size[0]) / c_im.shape[0],
                    float(init_target_size[1]) / c_im.shape[1],
                ]
        ...
        if c_im.shape != original_shape:
            v = [
                float(original_shape[0]) / c_im.shape[0],
                float(original_shape[1]) / c_im.shape[1],
            ]
            c_im = zoom(c_im, v, order=0)

        b_idx = np.nonzero(c_im >= 1)
        transformed_box_coords[b_num] = [
            b_idx[1].min(),
            b_idx[0].min(),
            b_idx[1].max(),
            b_idx[0].max(),
...
def box_coords_to_transformed(box_coords, proc_info, orig_shape=None):
    ...
    for b_num, box in enumerate(box_coords):
        ...
        assert (
            len(b_idx[0]) > 0
        ), "Bbx {} with coordinates '{}' could not be transformed".format(b_num, box)
        b_coords = [b_idx[1].min(), b_idx[0].min(), b_idx[1].max(), b_idx[0].max()]
        for v in range(
            4, box_coords.shape[-1]
        ):  # in case, for instance, box_coords also contain labels at the end
