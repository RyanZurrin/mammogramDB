...
class TestLoadAnnotationsGroup(object):
    def test_simple(self):
        ...
        annotations_group = simple_generator.load_annotations_group(
...
class TestFilterAnnotations(object):
    def test_simple_filter(self):
        input_annotations_group = [
            np.array([[0, 0, 10, 10], [150, 150, 50, 50]]),
        ]
        ...
        with pytest.warns(UserWarning):
            image_group, annotations_group = simple_generator.filter_annotations(
                [input_image], annotations_group, simple_generator.groups[0]
            )
    ...
    def test_complete(self):
        ...
        simple_generator = SimpleGenerator(
            input_annotations_group, image=input_image, num_classes=2
        )
        # expect a UserWarning
        with pytest.warns(UserWarning):
