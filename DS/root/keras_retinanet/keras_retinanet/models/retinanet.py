...
def default_classification_model(
    ...
    for i, n in enumerate(num_classes):
        ...
        outs = keras.layers.Reshape(
            (-1, n), name="pyramid_classification_reshape" + tag
        )(outs)
        outs = keras.layers.Activation(
            "sigmoid", name="pyramid_classification_sigmoid" + tag
        )(outs)
...
def default_regression_model(
    ...
    outputs = keras.layers.Conv2D(
        num_anchors * 4, name="pyramid_regression", **options
    )(outputs)
...
def __build_model_pyramid(name, model, features):
    ...
    if type(outs[0]) is list:
        ...
        for v in range(len(outs[0])):
            layers.append(
                keras.layers.Concatenate(axis=1, name=name + "_" + str(v))(
                    [l[v] for l in outs]
                )
            )
        return layers
...
def create_context_extractor():
    ...
    pooled_features_broadcast = keras.layers.Lambda(lambda x: x[0] + x[1])(
        [pooled_features, zerod_features]
    )
    context_features_concat = keras.layers.Concatenate()(
        [features, pooled_features_broadcast]
    )
    ...
    **kwargs
    ...
    if return_boxes:
        return keras.models.Model(
            inputs=model.inputs, outputs=[boxes, classification, anchors], name=name
        )
    else:
