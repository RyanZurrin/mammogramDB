...
def get_labels(metadata_dir, version="v4"):
    if version == "v4":
        boxable_classes_descriptions = os.path.join(
            metadata_dir, "class-descriptions-boxable.csv"
        )
        id_to_labels = {}
        cls_index = {}
    ...
    def __filter_data(self, labels_filter, fixed_labels):
        ...
        if fixed_labels:
            ...
            for l in labels_filter:
                label = str.lower(l)
                for v in [
                    v for v in self.id_to_labels.values() if label in str.lower(v)
                ]:
