...
class SummaryReport(Report):
    ...
    def __init__(self, save_dir, logger=None):
        ...
        self._dicoms_aggregated_file_name = (
            constants.SUMMARY_REPORT_DICOM_AGGREGATED_PKL
        )
        self.df_studies = None
        self.df_dicoms = None

        self.csv_header_studies = [
            "ix",
            "StudyInstanceUID",
            "output_dir",
            "input_dir",
    ...
    def _initialize(self):
        ...
        self.df_studies = pd.read_csv(
            studies_path,
            index_col=self.csv_header_studies[0],
            usecols=lambda x: x if x in self.csv_header_studies else False,
        )
        # Insert possibly missing columns
        for col in [
            c for c in self.csv_header_studies[1:] if c not in self.df_studies.columns
        ]:
            self.df_studies[col] = None

        # Same thing for df_dicoms
        self.df_dicoms = pd.read_csv(
            dicoms_path,
            index_col=self.csv_header_dicoms[0],
            usecols=lambda x: x if x in self.csv_header_dicoms else False,
        )
        for col in [
            c for c in self.csv_header_dicoms[1:] if c not in self.df_dicoms.columns
        ]:
    ...
    def generate(self, study_deploy_results, **additional_params):
        ...
        for sop_id, dicom in study_deploy_results.results["dicom_results"].items():
            ...
            assert (
                len(rows) == 1
            ), "Only one row expected for SOPInstanceUID={}; Got: {}".format(
                sop_id, rows
            )
            row = rows.iloc[0]
        ...
        if self.df_dicoms_aggregated is None:
            ...
            self.df_dicoms_aggregated = self.df_dicoms_aggregated.set_index(
                np.arange(len(self.df_dicoms_aggregated))
            )
        ...
        return {
            ...
            "dicoms_agg_path": dicoms_agg_path,
        }
