...
class MostMalignantImageReport(ImagesReport):
    ...
    def __init__(self, model, config, draw_box=True, logger=None):
        ...
        self.draw_modality_header_letters = (
            True  # Draw modality letters (LMLO, RCC, etc.)
        )
    ...
    def generate(
        ...
        images = super(self.__class__, MostMalignantImageReport).generate(
            self,
            study_deploy_results,
        ...
        row = study_deploy_results.metadata[
            study_deploy_results.metadata["SOPInstanceUID"] == iuid
        ].iloc[0]
        ...
        assert (
            img_info is not None
        ), "not image was generated with modality: {}, lat: {}, view: {}".format(
            modality, lat, view
        )
        img = img_info[1]
