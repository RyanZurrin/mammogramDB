...
class Report(object):
    ...
    def __init__(self, algorithm_version, logger=None):
        ...
        self._file_name_template = (
            "DH_{product}_{report_prefix}_{study_uid}.{file_extension}"
        )
    ...
    def get_output_file_name(self, study_instance_uid):
        ...
        Returns:
            str
        """
        return self._file_name_template.format(
            product=self._product.replace("-", ""),
    ...
    def exists(self, study_instance_uid, output_dir):
        return os.path.isfile(
            os.path.join(output_dir, self.get_output_file_name(study_instance_uid))
        )
