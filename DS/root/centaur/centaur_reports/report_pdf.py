...
class PDFReport(ImagesReport):
    ...
    def __init__(self, model, config, logger=None):
        ...
        self._product = (
            const.SAIGE_Q
            if config.get_run_mode() == const_deploy.RUN_MODE_CADT
            else const.SAIGE_DX
        )
    ...
    def max_length_texts(self):
        ...
        """
        return {"patient_info": 17, "slice_findings": 35}
    ...
    def generate(self, study_deploy_results, ram_images=None, clean_temp_files=True):
        ...
        def get_dbt_all_slices(laterality, view):
            ...
            l = (
                combined_results_df[
                    combined_results_df["SOPInstanceUID"] == instance_uid
                ]
                .sort_values("slice")["slice"]
                .unique()
            )
            if len(l) == 0:
                return "[-]"
        ...
        default_img_path = (
            ...
            + "/libs/pdf/dh_resources/dh_na_img2.png"
        )
        report_imgs_paths = defaultdict(lambda: default_img_path)
        instance_uids = {}
        for modality, lat, view in itertools.product(
            ("dxm", "dbt"), ("L", "R"), ("CC", "MLO")
        ):
            ...
            if maps[key] is not None:
                ...
                # Save the image
                img_name = self._config[
                    Config.MODULE_REPORTS, "temp_imgs_filename_template"
                ].format(modality=modality, laterality=lat, view=view)
        ...
        self._report_params[
            "dh-suspicious-level-case"
        ] = CategoryHelper.get_category_text(
            study_results["total"]["category"], run_mode=run_mode
        )
        self._report_params["dh-suspicious-level-left"] = (
            CategoryHelper.get_category_text(
                study_results["L"]["category"], run_mode=run_mode
            )
        ...
        self._report_params["dh-suspicious-level-right"] = (
            CategoryHelper.get_category_text(
                study_results["R"]["category"], run_mode=run_mode
            )
            if "R" in study_results
            else None
        )
        self._report_params["dh-study-percentile"] = study_results["total"][
            "postprocessed_percentile_score"
        ]
        ...
        for lat, view in itertools.product(("l", "r"), ("mlo", "cc")):
            ...
            if dbt is None:
                ...
                self._report_params[
                    "dh-findings-main-slice-dbt-{}-{}".format(lat, view)
                ] = self._report_params[
                    "dh-findings-all-slices-dbt-{}-{}".format(lat, view)
                ] = ""
            else:
                self._report_params[
                    "dh-findings-main-slice-dbt-{}-{}".format(lat, view)
    ...
    def _create_html(self):
        ...
        for tag in soup.find_all(id=re.compile("dh-findings")):
            tag.string = self._truncate_text(
                tag.string, self.max_length_texts["slice_findings"]
            )
