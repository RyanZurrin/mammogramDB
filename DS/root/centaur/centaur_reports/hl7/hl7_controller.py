...
class CADtHL7(BaseCentaurHL7):
    ...
    def generate_fail_obx5(self, results):
        if results.preprocessed and not results.passed_checker_acceptance_criteria():
            ...
            max_flags = 4
            error_message = "Error Code 002: Study does not pass acceptance criteria. Failed acceptance criteria: {}".format(
                failed_flags[:max_flags]
            )

        else:
            # Unexpected error
            error_message = (
                "Error Code 001: An unexpected error occurred during study processing. "
                "Please contact DeepHealth support for more information."
            )

        return self.OBX5_ERROR_TEMPLATE.format(
            study_instance_uid=self.study_instance_uid, error_message=error_message
        )
