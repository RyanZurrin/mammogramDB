...
import copy
import pydicom
from centaur_reports.helpers.helper_sr import (
    CodedEntry,
    DataEntity,
    get_image_attrs,
    value_container,
...
class SRWriter(object):
    def __init__(
        self, study_deploy_results, intended_workstation, algorithm_version, run_mode
    ):
        """
        ...
        assert run_mode in [
            CONST.RUN_MODE_CADX,
            CONST.RUN_MODE_CADT,
        ], "invalid run_mode {}".format(run_mode)
    ...
    def populate_sr(self):
        ...
        self.language_container = (
            self.get_lang_container()
        )  # indicate language information for this report (English)
        self.image_library = (
            self.get_image_library()
        )  # creates a list of images from on with get_finding_container will add annotation and other text
        self.findings_container = (
            self.get_findings_container()
        )  # box of text that has number of finding and Case Suspicion Level' asla add bounding boxes
        self.detection_summary = (
            self.get_detection_summary()
        )  # detection summary success? etc.
        self.analysis_summary = (
            self.get_analysis_summary()
        )  # state no analysis was attempted
        ...
        self.sr.ContentSequence = Sequence(
            [
                self.language_container,
                self.image_library,
                self.findings_container,
    ...
    def add_evidence_container(self):
        ...
        for series_instance_uid in self.metadata.SeriesInstanceUID.unique():
            ...
            for idx, row in self.metadata[
                self.metadata.SeriesInstanceUID == series_instance_uid
            ].iterrows():
    ...
    def get_lang_container(self):
        ...
        co.set_code(
            (
                self.report_info["Language"][0],
                "RFC3066",
                self.report_info["Language"][1],
            )
        )
        ...
        seq.set_code(
            (
                self.report_info["LanguageCountry"][0],
                "ISO3166_1",
                self.report_info["LanguageCountry"][1],
            )
        )
            ...
            if len(self.dicom_results[sop_instance_uid]) == 0:
                ...
                for box_idx in range(len(self.dicom_results[sop_instance_uid])):
                    ...
                    if DicomTypeMap.get_type_row(row) == DicomTypeMap.DBT:
                        sop_co.ReferencedFrameNumber = self.dicom_results[
                            sop_instance_uid
                        ][box_idx]["slice"]
                    image_co.co.ReferencedSOPSequence = Sequence([sop_co])
                    image_attrs = get_image_attrs(row, self.report_info)
    ...
    def apply_erad_modifications(self):
        ...
        for foruid in self.metadata.FrameOfReferenceUID.unique():
            ...
            assert (
                len(dbts) == 1
            ), "only expect one dbt per FORUID, however got these dbts {} for FORUID {}".format(
                dbts, foruid
            )
    ...
    def get_findings_container(self):
        ...
        if (
            self.run_mode == CONST.RUN_MODE_CADX
            and sum([len(self.dicom_results[a]) for a in self.dicom_results]) > 0
        ):
            with_findings = True
        ...
        annotations_list.append(
            value_container(
                "TEXT", ("111033", "DCM", "Impression Description"), lesion_detected
            ).val
        )
        ...
        if self.report_info["intended_workstation"] in ["ThreePalm", "eRad"]:
            ...
            certainty_co_val.set_name_code(
                ("DH0402", "DHCODE", "Case Assessment Value")
            )
            certainty_co_val.val.TextValue = CategoryHelper.get_category_abbreviation(
                self.category, self.run_mode
            )
            ...
            raise ValueError(
                "Config 'intended_workstation': {} not supported.".format(
                    self.report_info["intended_workstation"]
                )
            )
        ...
        annotations_list.append(
            value_container(
                ...
                self.report_info["AlgorithmName"],
            ).val
        )
        annotations_list.append(
            value_container(
                ...
                self.report_info["AlgorithmVersion"],
            ).val
        )
