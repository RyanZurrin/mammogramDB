    ...
    def generate(self, study_deploy_results, ram_images=None, **kwargs):
        ...
        if ram_images is not None:
            for key, value in ram_images.items():
                if "synth" in value:
                    synth_items.append(
                        {"metadata": metadata.loc[key], "synth_im": value["synth"]}
                    )
        else:
            bt_metadata = metadata[
                metadata.apply(DicomTypeMap.get_type_row, axis=1) == DicomTypeMap.DBT
            ].copy()
            bt_metadata["synth_path"] = bt_metadata["np_paths"].apply(
                lambda p: os.path.join(os.path.dirname(p), "synth", "frame_synth.npy")
            )
            for idx, row in bt_metadata.iterrows():
                assert os.path.isfile(row["synth_path"]), (
                    f'Unable to find synthetic for {row["SOPInstanceUID"]} in '
                    f'{row["synth_path"]}'
                )
                synth_items.append(
                    {"metadata": row, "synth_im": np.load(row["synth_path"])}
                )
