...
from DS.root.deephealth_utils.data import parse_logs
...
class Dashboard:
    def __init__(self, output_dir):
        ...
        assert (
            len(logs) == 1
        ), "Expected exactly one log file, found the following {}".format(logs)
        self.reader = parse_logs.LogReader(os.path.join(self.output_dir, logs[0]))
        configs = [
            f for f in os.listdir(self.output_dir) if re.search(r".*_config.json$", f)
        ]
        assert (
            len(configs) == 1
        ), "Expected exactly one config file, found the following {}".format(configs)
    ...
    def collect_data(self):
        ...
        progress_df = pd.DataFrame(
            {
                "Completed": [self.reader.n_files, self.reader.n_studies],
                "Remaining": [
                    self.reader.n_files - n_input_files,
                    self.reader.n_studies - n_input_dirs,
                ],
                "Total": [n_input_files, n_input_dirs],
            },
            index=["File", "Study"],
        )
        self.tables_dict = {
            "Files Acceptance Criteria Counts": self.reader.count_files_ac(),
    ...
    def view_web(self):
        ...
        def show_tables():
            ...
            return flask.render_template("view.html", tables=tables, titles=table_names)
