...
class StudyDeployResults(object):
    def __init__(
        self, results_file_name=const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
    ):
        ...
        self._checker_validation_passed = (
            False  # The study passed the Checker validations
        )
        self._failed_acceptance_criteria_list = (
            []
        )  # List of failed acceptance criteria list-str
        ...
        self.results_file_name = (
            results_file_name  # Name of the default results json name
        )

        self.reports_expected = (
            []
        )  # List of keys with the reports expected to be generated for this study
        self.reports_generated = (
            {}
        )  # Results for each report that should have been generated (dict of dicts)

        self.output_send_results = (
            OrderedDict()
        )  # Results of the output send operation for each output to be sent (OrderedDict of dicts)
    ...
    def is_completed_ok(self):
        ...
        """
        return (
            self.passed_checker_acceptance_criteria()
            and self.predicted
            and self.reports_generated_ok()
            and self.outputs_sent_ok()
            and not self.has_unexpected_errors()
        )
    ...
    def get_metadata(self, passed_checker_only=False):
        ...
        if self._metadata is None:
            return None
        return (
            self._metadata[self._metadata["failed_checks"].isnull()].copy()
    ...
    def set_checker_results(self, study_accepted, failed_flags):
        ...
        if not study_accepted:
            ...
            self._error_messages.append(
                "The study did not meet the required acceptance criteria. "
