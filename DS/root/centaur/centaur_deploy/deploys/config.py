...
class Config(object):
    ...
    def __init__(self, config_file_path=None, overriden_params={}):
        ...
        if config_file_path is not None:
            assert os.path.isfile(
                config_file_path
            ), f"Config file not found in {config_file_path}"
    ...
    def from_json_file(cls, json_file_path):
        ...
        Returns:
            Config object
        """
        assert os.path.isfile(json_file_path), "File {} not found".format(
            json_file_path
        )
    ...
    def get_algorithm_version(cls):
        ...
        """
        assert os.path.isfile(
            const_deploy.ALGORITHM_VERSION_PATH
    ...
    def get_product_label(cls, product):
        ...
        for line in label.split("\n"):
            ...
            if "[VERSION_PLACEHOLDER]" in line:
                ...
                num_spaces_now = num_spaces_before + (
                    len("[VERSION_PLACEHOLDER]") - len(algorithm_version)
                )
                line_to_add = (
                    line[:ix] + algorithm_version + (" " * num_spaces_now) + "||\n"
                )
    ...
    def set_param(self, param_name, value):
        ...
        assert (
            len(modules) > 0
        ), f"Parameter {param_name} was not found in any Config section"
    ...
    def set_cadt_operating_points(self, dxm, dbt, explicitly_set=False):
        ...
        """
        assert isinstance(dxm, float) and isinstance(
            dbt, float
        ), f"Expected 2 float values. Got: {dxm}, {dbt}"
        self[Config.MODULE_ENGINE, "cadt_operating_point_values"] = {
            "dxm": dxm,
            "dbt": dbt,
        }
    ...
    def _initialize(self):
        ...
        self[
            Config.MODULE_ENGINE, "cadt_operating_point_values"
        ] = None  # Dictionary of float ('Dxm', 'Dbt')

        self[Config.MODULE_REPORTS, "keep_temp_pdf_files"] = False

        self[Config.MODULE_REPORTS, "temp_html_files_dir"] = "temp_html_files"
        self[
            Config.MODULE_REPORTS, "temp_imgs_filename_template"
        ] = "{modality}-{laterality}-{view}.png"
    ...
    def _paths_to_absolute(self):
        ...
        """
        if (
            "input_dir" in self[Config.MODULE_IO]
            and self[Config.MODULE_IO, "input_dir"] is not None
        ):
            self[Config.MODULE_IO, "input_dir"] = os.path.realpath(
                self[Config.MODULE_IO, "input_dir"]
            )
        if (
            "output_dir" in self[Config.MODULE_IO]
            and self[Config.MODULE_IO, "output_dir"] is not None
        ):
            self[Config.MODULE_IO, "output_dir"] = os.path.realpath(
                self[Config.MODULE_IO, "output_dir"]
            )
    ...
    def _sync_params(self):
        ...
        self[Config.MODULE_ENGINE, "parallelize"] = self[
            Config.MODULE_DEPLOY, "parallelize"
        ]
    ...
    def __getitem__(self, item):
        ...
        assert (
            len(item) == 2 and item[0] in Config.MODULES and isinstance(item[1], str)
        ), (
            'Expected format: "config[Config.MODULE_X, \'param\'] = value" or just "config[Config.MODULE_X]"'
            ", where 'MODULE_X' in {}".format(Config.MODULES)
        )
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(key, tuple):
            ...
            assert isinstance(
                value, dict
            ), "Expected a dictionary of parameters for the module '{}'".format(key)
            self._params[key] = value
        else:
            assert (
                len(key) == 2 and key[0] in Config.MODULES
