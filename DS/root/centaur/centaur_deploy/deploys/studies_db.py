...
class StudiesDB(object):
    CODE_MAPPINGS = {
        ...
        (
            "results_status_code",
            1,
        ): "Study was not successfully processed due to an unexpected error",
        (
            "results_status_code",
            2,
        ): "Study was not processed due to failure of acceptance criteria",
        ("sending_status_code", -1): "Outputs not attempted to send yet",
        (
            "sending_status_code",
            0,
        ): "Outputs successfully received by client clinical systems",
        (
            "sending_status_code",
            1,
        ): "Outputs not successfully received by client clinical systems",
    }
    ...
    def __init__(self, db_path, code_mappings=None):
        ...
        self._code_mappings = (
            self.CODE_MAPPINGS if code_mappings is None else code_mappings
        )
        self._initialize()
    ...
    def insert_study(self, input_path):
        ...
        """
        sql = (
            f"INSERT INTO {self.studies_table} ("
        ...
        params = (
            ...
            0,
            self._code_mappings[("processing_status_code", 0)],
            -1,
            self._code_mappings[("results_status_code", -1)],
            -1,
            self._code_mappings[("sending_status_code", -1)],
        )
    ...
    def mark_study_as_failed(self, input_path, error_message):
        ...
        if study["processing_status_code"] == 0:
            ...
            params = {
                ...
                "processing_status_text": self.CODE_MAPPINGS[
                    ("processing_status_code", processing_status_code)
                ],
                "results_status_code": results_status_code,
                "results_status_text": self.CODE_MAPPINGS[
                    ("results_status_code", results_status_code)
                ],
            ...
            warnings.warn(
                f"The study {input_path} was not marked as failed because "
    ...
    def _initialize(self):
        ...
        if osp.isfile(self._db_path):
            ...
            assert osp.isdir(
                folder
            ), f"Folder {folder} does not exist. The database cannot be created"
    ...
    def _run_non_query(self, sql, params=None, single_row_operation=False):
        ...
        try:
            ...
            if single_row_operation:
                ...
                assert (
                    c.rowcount == 1
                ), "{} rows would be affected by this change (single row expected)".format(
                    c.rowcount
                )
            conn.commit()
            return c.rowcount
