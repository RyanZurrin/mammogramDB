...
class DirMonitorInput(Input):
    def __init__(
        ...
        studies_db,
        logger=None,
        new_study_threshold_seconds=None,
        sleep_for_seconds=None,
    ):
        """
        Constructor
        Args:
        ...
        self._sleep_for_seconds = (
            sleep_for_seconds
            if sleep_for_seconds is not None
    ...
    def get_next_study(self):
        ...
        """
        self.logger.info(
            log_line(-1, "Starting Input Dir Monitor. Waiting for new studies...")
        )
        while True:
            ...
            if next_study is None:
                self.logger.debug(
                    log_line(
                        -1, "No new studies found in DB. Checking the input folder..."
                    )
                )
                ...
                if num_new_studies == 0:
                    self.logger.debug(
                        log_line(
                            -1,
                            f"No new studies found in the input dir. "
                ...
                self.logger.debug(
                    log_line(-1, f"A new study was read from StudiesDB:\n{next_study}")
                )
                ...
                try:
                    ...
                    for f in glob.glob(input_path + "/*"):
                        ...
                        assert os.path.isfile(
                            f
                        ), f"{f} does not exist or it does not seem to be a regular file."
                        file_list.append(f)
                    ...
                    self.logger.info(
                        log_line(
                            -1,
                            f"New study read from StudiesDB: {study_name}; Ix: {index}; Filelist: {file_list}",
                        )
                    )
                    ...
                    self.logger.warning(
                        log_line(-1, "Studies generator stopped externally")
                    )
                    break
                except:
                    # IO Error. Mark the study as failed in StudiesDB so that it's not processed again
                    error_message = (
                        "Unexpected I/O error when processing the following path: "
    ...
    def check_new_studies(self):
        ...
        num_new_studies = 0
        self.logger.debug(
            log_line(-1, f"Searching for new studies in {self.input_dir}...")
        )
        ...
        if len(folder_names) > 0:
            self.logger.debug(
                log_line(
                    -1,
                    f"Found {len(folder_names)} studies to look at in the input folder",
                )
            )
            for folder_name in folder_names:
                ...
                if self.use_transfer_complete_file:
                    ...
                    if os.path.isfile(
                        os.path.join(study_dir, const.TRANSFER_COMPLETE_FILE_NAME)
                    ):
                        self.logger.debug(
                            log_line(
                                -1,
                                f"{const.TRANSFER_COMPLETE_FILE_NAME} found."
                        ...
                        if inserted:
                            num_new_studies += 1
                            self.logger.info(
                                log_line(-1, f"{study_dir} INSERTED in StudiesDB")
                            )
                        else:
                            self.logger.debug(
                                log_line(
                                    -1, f"{study_dir} was NOT inserted in StudiesDB"
                                )
                            )
                    else:
                        self.logger.debug(
                    ...
                    if latest_timestamp is None:
                        self.logger.warning(
                            log_line(5, f"Files not found in folder {folder_name}")
                        )
                        continue
                    self.logger.debug(
                        log_line(-1, f"Latest Timestamp found: {latest_timestamp}")
                    )
                    ...
                    if diff_in_seconds > self.new_study_threshold_seconds:
                        self.logger.debug(
                            log_line(-1, f"Attempting to insert {study_dir} in DB...")
                        )
                        ...
                        self.logger.debug(
                            log_line(
                                -1,
                                f"{diff_in_seconds}<{self.new_study_threshold_seconds}."
                                " Nothing to do yet",
                            )
                        )
