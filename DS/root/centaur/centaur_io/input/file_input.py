...
class FileInput(Input):
    ...
    def get_ingress(self):
        if os.path.isfile(self.ingress):
            if not any(
                [self.ingress.endswith(ext) for ext in [".txt", ".pkl", ".csv"]]
            ):
                raise IOError(
                    f"Input file {self.ingress} does not have a supported format (please make sure that "
            ...
            if self.ingress.endswith(".txt"):
                ...
                assert (
                    "file_path" in input_df.columns
                ), f"The file_path column is missing from {self.ingress}"
            ...
            raise IOError(
                "Directory {} does not exist, not processing.".format(self.ingress)
            )
    ...
    def count_studies(self):
        ...
        return len(
            [
                file
                for file in os.listdir(self.ingress)
                if os.path.isdir(os.path.join(self.ingress, file))
            ]
        )
    ...
    def count_files(self):
        ...
        return len(
            [
                f
                for f in glob.glob(os.path.join(self.ingress, "**"), recursive=True)
                if os.path.isfile(f)
            ]
        )
