...
class InputDirMonitorThread(Thread):
    def __init__(
        ...
        input_dir,
        studies_db,
        new_study_threshold_seconds,
        sleep_for_seconds=10,
        use_transfer_complete_file=False,
        logger=None,
    ):
        """
        Constructor
    ...
    def run(self):
        ...
        """
        self.logger.info(
            log_line(-1, f"Listening for new studies in {self.input_dir}...")
        )
        try:
            while True:
                ...
                if len(folder_names) > 0:
                    self.logger.info(
                        log_line(
                            -1, f"Found {len(folder_names)} new studies to process"
                        )
                    )
                    for folder_name in folder_names:
                        ...
                        if self.use_transfer_complete_file:
                            ...
                            if os.path.isfile(
                                os.path.join(
                                    study_dir, constants.TRANSFER_COMPLETE_FILE_NAME
                                )
                            ):
                                self.logger.info(
                                    log_line(
                                        -1,
                                        f"{constants.TRANSFER_COMPLETE_FILE_NAME} found."
                                        f" Attempting to insert {study_dir} in DB...",
                                    )
                                )
                                inserted = self.studies_db.insert_if_not_exist(
                                    study_dir
                                )
                                self.logger.info(
                                    log_line(
                                        -1, f"{study_dir} inserted in DB: {inserted}"
                                    )
                                )
                            else:
                                self.logger.info(
                            ...
                            latest_timestamp = self.get_latest_timestamp_in_dir(
                                study_dir
                            )
                            if latest_timestamp is None:
                                self.logger.warning(
                                    log_line(
                                        5, f"Files not found in folder {folder_name}"
                                    )
                                )
                                continue
                            self.logger.info(
                                log_line(
                                    -1, f"Latest Timestamp found: {latest_timestamp}"
                                )
                            )
                            ...
                            if diff_in_seconds > self.new_study_threshold_seconds:
                                self.logger.info(
                                    log_line(
                                        -1, f"Attempting to insert {study_dir} in DB..."
                                    )
                                )
                                ...
                                self.logger.info(
                                    log_line(
                                        -1,
                                        f"Study is too recent ({diff_in_seconds} seconds old)",
                                    )
                                )
                else:
                ...
                self.logger.info(
                    log_line(-1, f"Sleeping for {self.sleep_for_seconds} seconds...")
                )
            ...
            self.logger.exception(
                "Unexpected error in the input monitor thread", exc_info=True
            )
            raise
