...
def get_orientation_change(image_lat, patient_orientation):
    ...
    assert (
        image_lat in valid_image_laterality
    ), "invalid value for image_validation:{}".format(image_lat)
    assert all(
        x in valid_patient_orientation for x in patient_orientation
...
def implement_orientation(pixel_array, orientation_change):
    ...
    # orientation change one of ["rotate_180", "left_right_flip', "up_down_flip", "identity", None]
        ...
        raise ValueError(
            "orientation change change value of {} is unexpected".format(
                orientation_change
            )
        )
    assert (
        old_pixel_shape == pixel_array.shape
    ), "orientation changes should not change pixel shape, original: {} new:{}".format(
        old_pixel_shape, pixel_array.shape
    )
    return pixel_array
...
def set_pixel_array(args, return_dict=None):
    ...
    orientation_change = get_orientation_change(
        dr.metadata["ImageLaterality"], dr.metadata["PatientOrientation"]
    )
