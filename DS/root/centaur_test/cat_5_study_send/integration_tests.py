...
from DS.root.centaur.centaur_io.output.pacs_output import PACSOutput
from DS.root.deephealth_utils.data import format_dicom_tag_str
from DS.root.deephealth_utils.data.dicom_utils import compare_dicoms
...
def send_hl7_message(listener_ip, listener_port, study=DataManager.STUDY_01_HOLOGIC):
    ...
    assert os.path.isfile(
        results_file_path
    ), f"HL7 baseline message not found in {results_file_path}"
...
def send_dicom(dicom_filepath, ip, port, aetitle):
    print(
        f"Sending DICOM {dicom_filepath} to a listener with ip={ip}, port={port}, and aetitle={aetitle}"
    )
...
def test_T_192(run_mode, dicom_listener_ip, dicom_listener_port):
    ...
    PREFFIX = "+B"
    AETITLE = "DEEPHEALTH"
    file_preffix_preview = (
        "SC"  # The test is done with Dicom Reports, which are SC images
    )
    file_preffix_sr = "SRm"
    ...
    if is_local_dicom_listener:
        ...
        assert os.path.isdir(
            DICOM_RECEIVE_FOLDER
        ), f"Folder {DICOM_RECEIVE_FOLDER} needs to be mounted externally"
    ...
    for study in studies:
        ...
        baseline_results_path = os.path.join(
            base_dir, study, const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
        )
        ...
        if run_mode == const_deploy.RUN_MODE_CADT:
            if study in studies_suspicious:
                preview_dicom_filepath = baseline_result.reports_generated[
                    Report.CADT_PREVIEW
                ]["dcm_output"]
            else:
                preview_dicom_filepath = None
        elif run_mode == const_deploy.RUN_MODE_CADX:
            preview_dicom_filepath = baseline_result.reports_generated[Report.PDF][
                "dcm_output"
            ]
        else:
        ...
        if preview_dicom_filepath is not None:
            assert os.path.isfile(
                preview_dicom_filepath
            ), f"{preview_dicom_filepath} does not exist"
        ...
        for file_path, file_preffix in zip(
            [preview_dicom_filepath, sr_path], [file_preffix_preview, file_preffix_sr]
        ):
            ...
            dicom_filepath_received = (
                f"{DICOM_RECEIVE_FOLDER}/{PREFFIX}_{study_uid}/{file_preffix}.{sop_uid}"
            )
            assert os.path.isfile(
                dicom_filepath_received
            ), f"{dicom_filepath_received} not received at the listener side"
            ...
            assert (
                len(diffs) == 0
            ), f"{preview_dicom_filepath} and {dicom_filepath_received} have differences"
