...
START_DEFAULT_VALUES = {
    "save_to_ram": True,
    "cadt_operating_point_key": "balanced",
...
def get_container_name(working_dir):
    ...
    assert len(data) <= 1, "expect 1 or 0 line in execution_log got {}".format(
        len(data)
    )
    ...
    assert words[0] == "START:", (
        "expected the first word of the execution_log "
...
def helper(
    working_dir, centaur_client_dir, centaur_support_dir, docker_image, mode=None
):
    """
    ...
    """
    assert os.path.exists(centaur_support_dir), "{} doesnt exist".format(
        centaur_support_dir
    )

    # set up folder and call the start.sh command
    make_start_up_dir(
        working_dir, centaur_client_dir, centaur_support_dir, docker_image
    )
    ...
    if mode == "invalid":
        ...
        val = subprocess.call(
            f"{bash_dir}/start.sh --operating_point high_spec --save_to_disk --remove_input_files",
            shell=True,
        )
    else:
    ...
    try:
        ...
        for key in test_keys:
            if key in ("pacs_receive_port", "pacs_send_port"):
                assert (
                    config["IO"][key] == 1
                ), "{} was not updated based on client config.json"
            else:
                assert (
                    config["IO"][key] == "test"
                ), "{} was not updated based on client config.json"
        ...
        assert not os.path.exists(
            working_dir
        ), "temp folder {} was not deleted sucessfully".format(working_dir)
...
def test_T_E_214_2(centaur_client_dir, centaur_support_dir, docker_image):
    ...
    try:
        ...
        assert (
            val == 1
        ), "start.sh should exit with error if invalid arguments are passed in "

    finally:
        clean_up(working_dir)
        assert not os.path.exists(
            working_dir
        ), "temp folder {} was not deleted successfully".format(working_dir)
...
def test_T_E_214_3(centaur_client_dir, centaur_support_dir, docker_image):
    ...
    try:
        ...
        assert (
            config["ENGINE"]["cadt_operating_point_key"]
            == START_DEFAULT_VALUES["cadt_operating_point_key"]
        ), (
        ...
        assert (
            config["IO"]["remove_input_files"]
            == START_DEFAULT_VALUES["remove_input_files"]
        ), "default value for remove_input_files Should be {} ".format(
            START_DEFAULT_VALUES["remove_input_files"]
        )
