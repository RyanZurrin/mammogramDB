    ...
    for repo, values in states.items():
        ...
        current_commit = (
            subprocess.check_output(
                "git --git-dir={} rev-parse --verify HEAD".format(git_dir), shell=True
            )
            .decode("utf-8")
            .strip()
        )
        current_branch = (
            subprocess.check_output(
                "git --git-dir={} rev-parse --abbrev-ref HEAD".format(git_dir),
                shell=True,
            )
            .decode("utf-8")
            .strip()
        ...
        if current_branch not in ["dev", "master"]:
            print(
                "Warning: the current branch '{}' for repo {} is not dev or master".format(
                    current_branch, repo
                )
            )
        if branch != current_branch:
            raise IOError(
                "Git repo {} current has branch {}; does not match required branch {}".format(
                    repo, current_branch, branch
                )
            )
        if commit_id != current_commit:
            raise IOError(
                "Git repo {} current has commit {}; does not match required commit {}".format(
                    repo, current_commit, commit_id
                )
            )
...
def check_hashes(states):
    for path, sha1_sums in states.items():
        ...
        if not isinstance(sha1_sums, list):
            sha1_sums = [sha1_sums]
        current_sha1 = (
            subprocess.check_output("sha1sum {}".format(path), shell=True)
            .decode("utf-8")
            .strip()
            .split(" ")[0]
        )
        ...
        if all([x != current_sha1 for x in sha1_sums]):
            raise IOError(
                "File {} has sha1sum {}; does not match required sha1sums {}".format(
                    path, current_sha1, sha1_sums
                )
            )
