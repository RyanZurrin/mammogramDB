...
def test_T_182():
    ...
    assert (
        avail_gb >= constants.MIN_HARD_DRIVE_STORAGE_GB
    ...
    try:
        ...
        for gpu in gpus:
            assert (
                gpu.memory_limit / 2**30 >= constants.MIN_MEMORY_GPU_GB
    ...
    try:
        proc = subprocess.run(
            "nvcc --version", capture_output=True, text=True, check=False, shell=True
        )
        assert (
            proc.stderr == ""
        ), "Unexpected error when running 'nvcc --version': {}".format(proc.stderr)
        version_info = proc.stdout
        r = re.search("V\d{1,2}(.)*$", version_info, flags=re.MULTILINE)
        assert r is not None, "CUDA version not found. Full version info: {}".format(
            version_info
        )
        version = version_info[r.span()[0] + 1 : r.span()[1]]
        print("CUDA version: {}".format(version))
        assert Version(version) >= Version(
            constants.CUDA_MIN_VERSION
        ), "Expected CUDA>={}. Computed: {}. Full version info:\n{}".format(
            constants.CUDA_MIN_VERSION, version, version_info
        )
    except AssertionError:
        raise
    except:
    ...
    try:
        ...
        assert (
            proc.stderr == ""
        ), "Unexpected error when running 'cat /proc/driver/nvidia/version': {}".format(
            proc.stderr
        )
        ...
        assert Version(v) >= Version(
            constants.NVIDIA_DRIVERS_MIN_VERSION
        ), "Expected nvidia drivers version>={}. Computed: {}. Full version info:\n{}".format(
            constants.NVIDIA_DRIVERS_MIN_VERSION, v, version_info
        )
    except AssertionError:
        raise
    except:
