...
from DS.root.centaur.centaur_engine.helpers.helper_results_processing import (
    combine_foruid_boxes,
    assign_cadx_categories,
)
        ...
        for mode in all_files[foruid]:
            ...
            if mode in mode_exclude:
                for box in boxes:
                    ...
                    ), "dicom {} has unique FORUID but have boxes from {}".format(
                ...
                if boxes_foruid is None:
                    ...
                    for box in boxes:
                        score = box["score"]
                        assert (
                            score in boxes_foruid
                        ), "dicoms {} and {} have same FORUID however they dont have the same combined boxes".format(
                            dicom, foruid_match
                        )
...
def test_T_217():
    ...
    study_path_results = os.path.join(
        baseline_path, const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
    )
    ...
    cadx_threshold_config = load_config(
        const.CADX_THRESHOLD_PATH + "/" + "{}.json".format(cadx_version)
    )
    model_results = assign_cadx_categories(
        model_results, metadata, cadx_threshold_config
    )

    model_version = helper_model.get_actual_version_number("model", "current")
    model_config = load_config(
        const.MODEL_PATH + str(model_version) + "/" + const.MODEL_CONFIG_JSON
    )
    ...
    results = combine_foruid_boxes(
        model_results_all, metadata_all, model_config, pixel_data
    )
    ...
    results = combine_foruid_boxes(
        model_results_cview_dbt, metadata_cview_dbt, model_config, pixel_data
    )
    check_combine(all_files, results["dicom_results"], box_to_dicom, exclude)
    ...
    results = combine_foruid_boxes(
        model_results_cview_ffdm, metadata_cview_cview_ffdm, model_config, pixel_data
    )
    check_combine(all_files, results["dicom_results"], box_to_dicom, exclude)
