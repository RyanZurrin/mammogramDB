...
def test_T_3_MI01():
    ...
    manual_output_dir = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "manual_integration_output"
    )
    # Make sure it doesn't exist
    assert not os.path.isdir(
        manual_output_dir
    ), "Path 'manual_integration_output' already exists."
    ...
    results_dict = results_parser.load_results_json(
        os.path.join(
            centaur_output_path, DataManager.STUDY_03_DBT_HOLOGIC, "results.json"
        )
    )
    metadata = results_parser.get_metadata(results_dict)

    for idx, row in metadata.iterrows():
        boxes_info = results_dict["model_results"]["dicom_results"][row.SOPInstanceUID][
            "none"
        ]
        ...
        assert cv2.imwrite(
            os.path.join(
                manual_output_dir, "image_plots", "{}.png".format(row.SOPInstanceUID)
            ),
            img,
        )
