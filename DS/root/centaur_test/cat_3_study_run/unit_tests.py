...
from centaur_engine.helpers import (
    helper_model,
    helper_results_processing,
    helper_preprocessor,
)
...
def test_T_162(run_mode):
    ...
    config = Config()
    max_bbxs_displayed_total = config[
        Config.MODULE_ENGINE, "results_pproc_max_bbxs_displayed_total"
    ]
    max_bbxs_displayed_intermediate = config[
        Config.MODULE_ENGINE, "results_pproc_max_bbxs_displayed_intermediate"
    ]
    min_relative_bbx_size_allowed = config[
        Config.MODULE_ENGINE, "results_pproc_min_relative_bbx_size_allowed"
    ]
    max_relative_bbx_size_allowed = config[
        Config.MODULE_ENGINE, "results_pproc_max_relative_bbx_size_allowed"
    ]

    # OK case (after capping the number of bounding boxes per image)
    if run_mode == const_deploy.RUN_MODE_CADX:
        model_results = helper_results_processing.cap_bbx_number_per_image(
            model_results_orig,
    ...
    helper_results_processing.model_results_sanity_checks(
        model_results,
        study_deploy_results.results_raw,
        metadata_df,
        max_bbxs_displayed_total,
    ...
    instance_uid_3d = metadata_df[
        metadata_df.SOPClassUID == DicomTypeMap.get_dbt_class_id()
    ].iloc[0]["SOPInstanceUID"]
...
def test_T_226(run_mode):
    ...
    config_factory = ConfigFactory.VerificationInternalConfigFactory(
        input_dir=input_dir, output_dir=output_dir, optional_params_dict=baseline_params
    )
    ...
    results_file = os.path.join(
        output_dir, study_name, const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
    )
    ...
    assert (
        results.input_files == []
    ), f"Expected empty list for input files. Got: {results.input_files}"
    assert results.input_dir == os.path.join(
        input_dir, study_name
    ), f"Input dir doesn't match (Expected: os.path.join(input_dir, study_name); Got: {results.input_dir}"
    assert (
        results.get_study_dir_name() == study_name
    ), f"Study name doesn't match. Expected {study_name}; Got: {results.get_study_dir_name()}"
    assert (
        results.results_raw is None
    ), f"results_raw expected to be None. Got: {results.results}"
    assert (
        results.results is None
    ), f"Results expected to be None. Got: {results.results}"
...
def test_T_3_U04():
    ...
    results_parser.get_proc_info(results)[instance_uid]["original_shape"] = [
        size_y,
        size_x,
    ]
    ...
    model_results = results_processing.fix_bbxs_size(
...
def test_T_3_U06():
    ...
    model_results_test = results_processing.fix_bbxs_size(
        model_results, min_relative_bbx_size_allowed, max_relative_bbx_size_allowed
    )
    ...
    assert (
        model_results_test["dicom_results"][instance_uid]["none"][0]["coords"]
        == coords_expected
    )
...
def test_T_211():
    ...
    for study in (dm.STUDY_01_HOLOGIC, dm.STUDY_03_DBT_HOLOGIC):
        ...
        ), f"Study {study} expected to be suspicious"
        ...
        with open(
            f"{const_eng.CADT_THRESHOLD_PATH}/{original_thresholds_version}.json", "r"
        ) as f:
            original_thresholds = json.load(f)
        op_point_key = "balanced"
        cadt_operating_points_keys = {
            k: v[op_point_key] for k, v in original_thresholds.items()
        }
        model_results = results_processing.assign_cadt_categories(
            baseline_model_results, study_type, cadt_operating_points_keys
        )

        for lat in baseline_model_results["study_results"]:
            assert (
                model_results["study_results"][lat]["category"]
                == baseline_model_results["study_results"][lat]["category"]
            )

        # Modify thresholds
        new_thresholds = {k: 0.999 for k in cadt_operating_points_keys}
        model_results_2 = results_processing.assign_cadt_categories(
            baseline_model_results, study_type, new_thresholds
        )

        # Ensure with the new thresholds the study would always be not suspicious
        for lat in model_results_2["study_results"]:
            assert (
                model_results_2["study_results"][lat]["category"]
                == CategoryHelper.NOT_SUSPICIOUS
            )
...
def test_T_211_2():
    ...
    p = os.path.join(
        ...
        data_manager.STUDY_03_DBT_HOLOGIC,
        const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON,
    )
    ...
    config_default_params = ConfigFactory.VerificationInternalConfigFactory(
        optional_params_dict=cl_default_params
    ).params_dict
    ...
    results = engine._results_post_processing(
        study_deploy_results.results_raw, metadata_df
    )
    # Make sure the study is still Suspicious with the original conditions
    assert (
        results_parser.get_study_results(results)["total"]["category"]
        == CategoryHelper.SUSPICIOUS
    )
    ...
    config_params = ConfigFactory.VerificationInternalConfigFactory(
        optional_params_dict=cl_params_2
    ).params_dict
    ...
    assert (
        config[Config.MODULE_ENGINE, "cadt_operating_point_values"]
    ...
    results_parser.get_study_results(results2)["total"]["score"] = (
        config[Config.MODULE_ENGINE, "cadt_operating_point_values"][DicomTypeMap.DBT]
        - 0.01
    )
    engine = deployer.engine
    ...
    config_params = ConfigFactory.VerificationInternalConfigFactory(
        optional_params_dict=cl_params_3
    ).params_dict
    config = Config(overriden_params=config_params)
