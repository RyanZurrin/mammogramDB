...
def run_deployer(config_dict):
    ...
    deployer.logger.info(
        parse_logs.log_line(-1, "Initialization finished. Deploying...")
    )
    processed_studies = deployer.deploy(return_results=True)
    return processed_studies


def compare_dicom_results(
    model_results1, model_results2, orientation_map_original, orientation_map_new
):
    """
        ...
        for i, box in enumerate(model_results1["dicom_results"][dicom]["none"]):
            for key in box:
                if key == "score":
                    ...
                    score2 = round(
                        model_results2["dicom_results"][dicom]["none"][i][key], 4
                    )
                    if score1 != score2:
                        return False
                    ...
                    coordinate2 = orient_bounding_box_coords(
                        model_results2["dicom_results"][dicom]["none"][i][key],
                        img_shape2,
                        oc2,
                    )
    ...
    try:
        ...
        for dicom in dicoms:
            ...
            if target_orientation_change == "keep original":
                ...
                pixel_correct = implement_orientation(
                    pixels, current_orientation_change
                )
                pixels_target = implement_orientation(
                    pixel_correct, target_orientation_change
                )
                target_po = orientation_to_po[lat][target_orientation_change]

                assert (
                    get_orientation_change(lat, "|".join(target_po))
                    == target_orientation_change
                ), "target_po {} does not give target orientation {}".format(
                    target_po, target_orientation_change
                )
        ...
        match = compare_dicom_results(
            result_original.results,
            result_variation.results,
            original_orientation_change,
            new_dicom_to_orientation_change,
        )
        assert (
            match
        ), " the boxes of the oriented study does not match the original study"
    finally:
