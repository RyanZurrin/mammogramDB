...
#             helpers.validate_study_output_folder(os.path.join(output_dir, study))
#
...
def test_T_169(run_mode):
    ...
    for study in studies:
        ...
        config = Config()
        pp = Preprocessor(
            config[Config.MODULE_ENGINE], logger=utils.get_testing_logger()
        )

        passed = pp.preprocess(dcm_files)
        reasons = pp.get_failed_checks()
        assert passed, "Unexpected flags when preprocessing the files: {}".format(
            reasons
        )
        ...
        if run_mode == const.RUN_MODE_CADT:
            model = ModelSelector.select(
                centaur_model_version, thresholds_version, None
            )
        elif run_mode == const.RUN_MODE_CADX:
            model = ModelSelector.select(
                centaur_model_version, None, thresholds_version
            )
        else:
            raise ValueError(
                f"Unexpected run mode {run_mode}."
        ...
        utils.validate_dicom_results_study(
            results["dicom_results"], results["study_results"], proc_info, metadata_df
        )
...
#             helpers.compare_model_results_to_baseline(study, predicted_results)
#
...
def test_T_170(run_mode):
    ...
    try:
        ...
        for study in studies:
            ...
            assert (
                valid
            ), "Study did NOT pass the Preprocessor validations. Errors: {}".format(
                engine.preprocessor.get_failed_checks()
            )
            df = engine.preprocessor.get_metadata()
            assert len(df) == len(
                file_list
            ), "Expected {} rows in the metadata. Got: {}".format(
                len(file_list), len(df)
            )
            ...
            assert results_parser.are_equal(
                engine.results_raw, baseline_results.results_raw
            )
...
#
#     try:
#         helpers.test_full_deploy(config, dm.baseline_dir_centaur_output)
#     finally:
#         shutil.rmtree(output_dir)
