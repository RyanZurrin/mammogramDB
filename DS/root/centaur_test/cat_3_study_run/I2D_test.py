...
def test_T_219(input_study_folder="/root/HologicHD"):
    ...
    try:
        ...
        study_path = os.path.join(
            const.LOCAL_TESTING_STUDIES_FOLDER, DataManager.STUDY_01_HOLOGIC
        )
        ...
        for f in os.listdir(original_study_input_dir):
            ...
            ds = dh_dcmread(p)
            if (
                DicomTypeMap.get_image_classification(ds)
                == DicomTypeMap.Classification.INTELLIGENT_2D
            ):
                arr = ds.pixel_array
                ...
                dxm_shell.ManufacturerModelName = ds.dh_getattribute(
                ...
                dxm_shell.FrameOfReferenceUID = ds.dh_getattribute(
                    "FrameOfReferenceUID"
                )
                ...
                dxm_shell.ReferringPhysicianName = ds.dh_getattribute(
                    "ReferringPhysicianName"
                )
                dxm_shell.FieldOfViewHorizontalFlip = ds.dh_getattribute(
                    "FieldOfViewHorizontalFlip"
                )
                dxm_shell.FieldOfViewOrigin = ds.dh_getattribute("FieldOfViewOrigin")
                dxm_shell.FieldOfViewRotation = ds.dh_getattribute(
                    "FieldOfViewRotation"
                )
                dxm_shell.WindowCenter = ds.dh_getattribute("WindowCenter")
                dxm_shell.WindowWidth = ds.dh_getattribute("WindowWidth")
                dxm_shell.WindowCenterWidthExplanation = ds.dh_getattribute(
        ...
        modified_results_path = os.path.join(
            output_dir, "modified", const.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
        )
        modified_results = StudyDeployResults.from_json(modified_results_path)
        assert (
            modified_results.predicted
        ), f"The study {modified_study_input_dir} could not be predicted"

        original_results_path = os.path.join(
            output_dir, "original", const.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
        )
        original_results = StudyDeployResults.from_json(original_results_path)
        assert (
            original_results.predicted
        ), f"The study {original_study_input_dir} could not be predicted"

        print(
            f"Comparing results in {original_results_path} to {modified_results_path}..."
        )

        assert results_parser.are_equal(
            original_results.results, modified_results.results
