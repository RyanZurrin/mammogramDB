...
import DS.root.deephealth_utils.misc.config as dh_config
...
def test_T_0_U01():
    ...
    config[Config.MODULE_IO, "input_dir"] = data_manager.get_input_dir(
    ...
    saved_config = Config.from_json_file(
        os.path.join(run_dir, const_deploy.CENTAUR_CONFIG_JSON)
    )
    ...
    assert (
        current_packages.strip() == run_packages.strip()
    ), "Conda packages are not correctly saved"
    ...
    git_commits = dh_config.load_config(
        os.path.join(run_dir, const_deploy.CENTAUR_COMMIT_FN)
    )
    ...
    with pytest.raises(ValueError):
        Run.load_run(
            run_num=run_num, model_version=config[Config.MODULE_ENGINE, "model_version"]
        )
