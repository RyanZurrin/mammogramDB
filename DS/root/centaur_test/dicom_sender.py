...
class DicomSender:
    def __init__(self, port, config_file_path=None):
        self.config_file_path = (
            os.path.realpath(os.path.dirname(__file__) + "/storescu.cfg")
    ...
    def send_study(self, study_path):
        ...
        response_proc = subprocess.Popen(
        ...
        assert not re.search(
            "^E: .*$", output, flags=re.MULTILINE
        ), "Unexpected error in storescu response:\n {}".format(output)
        assert not "Error" in output, (
            "It seems there's been an error in the storescu command.\n"
            f"Command: {command}\n"
            f"Output: {output}"
        )
