...
collect_ignore = [
    ...
    "helpers.py",
    "basic_pipeline.py",
    "cat_3_study_run/manual_integration_tests.py",
    "cat_3_study_run/I2D.py",
    "cat_0/unit_tests.py",
]
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        help="Path to the centaur client_dir repo",
    )
    parser.addoption(
        "--centaur_support_dir",
        default=default_support_dir,
        help="Path to the centaur_support repo",
    )
    parser.addoption(
        "--docker_image", default=default_docker_image, help="Docker image id"
    )
    parser.addoption(
        "--intermediate_outputs",
        default="/root/dh_test_results/intermediate_outputs",
...
def pytest_generate_tests(metafunc):
    ...
    if mode is None:
        ...
        ), "Run mode not allowed ({}). Allowed modes: {}".format(mode, allowed_modes)
        modes = [mode]
