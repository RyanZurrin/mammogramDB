...
def test_T_201():
    ...
    assert os.path.isdir(
        "/root/centaur_support"
    ), "/root/centaur_support should be mounted for this test"
    ...
    try:
        ...
        assert (
            returncode == 1
        ), f"return code 1 should be returned (got return code {returncode})"
        print("\n\n")
        ...
        assert (
            returncode == 0
        ), f"return code 0 should be returned (got return code {returncode})"
        # Ensure the result file has read-only permissions
        permissions = oct(os.stat(json_filepath).st_mode)[-3:]
        assert (
            permissions == "440"
        ), f"Expected permissions 440 (read only) for file {json_filepath}. Got: {permissions}"
        ...
        command = (
            f"python {VERIFIER_PYTHON_PATH} -j {json_filepath} -e {error_log_filepath}"
        )
        returncode = run_command_with_returncode(command, verbose=verbose)
        assert (
            returncode == 0
        ...
        assert set(paths_to_modify) == set(
            mismatch_paths
