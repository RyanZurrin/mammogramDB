...
def compare_dcmtk_pydicom(original_file, file_encoded, output_folder, language):
    ...
    assert (
        ds.file_meta.TransferSyntaxUID == language
    ), "wrong transfer syntax language expected {} got {}".format(
...
def helper_explicit_vr_little_enidan(ts_dicom, output_folder):
    ...
    match = compare_dcmtk_pydicom(
        ts_dicom, dcmfile_out, output_folder, UID_map["Explicit VR Little Endian"]
    )
...
def helper_implicit_vr_little_endian(ts_dicom, output_folder):
    ...
    dcm_conv_function(ts_dicom, dcmfile_out, option)
    match = compare_dcmtk_pydicom(
        ts_dicom, dcmfile_out, output_folder, UID_map["Implicit VR Little Endian"]
    )
    assert match, "pixels values from pydicom does not match that of DCMTK"
...
def helper_explicit_vr_big_endian(ts_dicom, output_folder):
    ...
    dcm_conv_function(ts_dicom, dcmfile_out, option)
    match = compare_dcmtk_pydicom(
        ts_dicom, dcmfile_out, output_folder, UID_map["Explicit VR Big Endian"]
    )
    assert match, "pixels values from pydicom does not match that of DCMTK"
...
def helper_deflated_explicit_vr_little_endian(ts_dicom, output_folder):
    ...
    match = compare_dcmtk_pydicom(
        ...
        dcmfile_out,
        output_folder,
        UID_map["Deflated Explicit VR Little Endian"],
    )
    assert match, "pixels values from pydicom does not match that of DCMTK"
...
def helper_rle_lossless(ts_dicom, output_folder):
    ...
    match = compare_dcmtk_pydicom(
        ts_dicom, dcmfile_out, output_folder, UID_map["RLE Lossless"]
    )
    assert match, "pixels values from pydicom does not match that of DCMTK"
...
def helper_jpeg_lossless_process_14_sv1(ts_dicom, output_folder):
    ...
    match = compare_dcmtk_pydicom(
        ts_dicom, dcmfile_out, output_folder, UID_map["JPEG Lossless (Process 14, SV1)"]
    )
    assert match, "pixels values from pydicom does not match that of DCMTK"
...
def helper_j2k(ts_dicom, output_folder):
    ...
    assert (
        ds_encoded.file_meta.TransferSyntaxUID == UID_map["JPEG2000 Lossless"]
    ), "wrong Transfer syntax expected {} got {}".format(
        "JPEG2000 Lossless", UID_map[str(ds_encoded.file_meta.TransferSyntaxUID)]
    )
...
def helper_jpeg_ls_lossless(ts_dicom, output_folder):
    ...
    assert (
        ds_encoded.file_meta.TransferSyntaxUID == UID_map["JPEG LS Lossless"]
    ), "wrong Transfer syntax expected {} got {}".format(
        "JPEG2000 Lossless", UID_map[str(ds_encoded.file_meta.TransferSyntaxUID)]
    )
    pix_orig = ds_orig.pixel_array
