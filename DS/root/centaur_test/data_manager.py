...
import DS.root.centaur_test.constants as const
...
class DataManager(object):
    ...
    STUDY_05_HOLOGIC_COMBO_MODE = (
        "study_05_hologic_combo_mode"  # Hologic Combo mode (FFDM + DBT available)
    )

    def __init__(
        self,
        dataset_name="DS_01",
        test_data_local_mode=True,
        data_version=None,
        ...
        if test_data_local_mode:
            ...
            self._data_dir = osp.join(
                const_deploy.ROOT_PATH, const.CENTAUR_TEST_DATA_FOLDER
            )
            self._baseline_dir = osp.join(
                const_deploy.ROOT_PATH, const.CENTAUR_TEST_BASELINE_FOLDER
            )
        else:
            assert data_version is not None, "Data version not found"
            self._data_dir = osp.join(
                const.TEST_DATA_FOLDER, dataset_name, data_version
            )
            self._baseline_dir = (
                osp.join(const.TEST_BASELINE_FOLDER, dataset_name, baseline_version)
    ...
    def get_valid_studies(self, run_mode=None):
        ...
        if self.dataset_name == "DS_01":
            return [
                self.STUDY_01_HOLOGIC,
                self.STUDY_03_DBT_HOLOGIC,
                self.STUDY_04_HOLOGIC,
                self.STUDY_05_HOLOGIC_COMBO_MODE,
            ]
        raise NotImplementedError(
            "I don't have a way to know which studies should pass the Checker"
        )
    ...
    def baseline_dir(self):
        ...
        if self._baseline_dir is None:
            raise AssertionError(
                "The baseline directory is being requested, but no baseline version was specified."
            )
    ...
    def get_baseline_folder_args_suffix(cls, args_dict=None):
        ...
        if args_dict:
            return "__".join(
                map(lambda item: f"{item[0]}__{item[1]}", sorted(args_dict.items()))
            )
        return "DEFAULT"
    ...
    def get_filelist_df(self):
        ...
        if self.__filelist_df is None:
            ...
            assert osp.isfile(filelist_path), (
                "The filelist for the dataset could not be found in " + filelist_path
            )
            # Read the files
