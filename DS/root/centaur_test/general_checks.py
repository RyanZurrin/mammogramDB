...
def test_external_packages():
    try:
        ...
        traceback.print_exc()
        pytest.fail(
            "google cloud storage could not be imported. Try running pip install google-cloud-storage. Check https://cloud.google.com/python/setup for additional info"
        )
...
def test_pytest_html():
    try:
        ...
        traceback.print_exc()
        pytest.fail(
            "pytest-html not installed. You cannot generate html reports. Try pip install pytest-html"
        )
...
def test_data_files():
    ...
    assert (
        "md5" in df.columns
    ), "'md5' column could not be found in the filelist dataframe. File columns: {}".format(
        df.columns.to_list()
    )
    ...
    for key, row in df.iterrows():
        ...
        assert len(local_paths) == 1 and os.path.isfile(
            local_paths[0]
        ), "Error in row:\n{}.\nLocal path obtained: {}".format(row, local_paths)
        local_path = local_paths[0]
        local_md5 = data_manager.md5(local_path)

        assert (
            local_md5 == row["md5"]
        ), "MD5 signature does not match for file {}. Expected: {}; Calculated: {}".format(
            local_path, row["md5"], local_md5
        )
