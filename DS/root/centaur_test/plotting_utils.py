...
from DS.root.deephealth_utils.data.utils import dcm_to_preprocessed_array
from DS.root.centaur.centaur_engine.helpers.helper_preprocessor import get_orientation_change
from DS.root.centaur.centaur_engine.helpers.helper_results_processing import (
...
def plot_dicom(
    ...
    if isinstance(img, str):
        ...
        if is_dbt:
            assert (
                pixels.ndim == 3
            ), "Expected number of dimensions for DBT pixels to 3, \
            got {} dimensions instead".format(
                pixels.ndim
            )
...
def plot_box_on_synthetics(
    ...
    results_processed = results_parser.load_results_json(
        os.path.join(processed_study.study_output_dir, "results.json")
    )
    results_raw = results_parser.load_results_json(
        os.path.join(processed_study.study_output_dir, "results_raw.json")
    )
    ...
    for dicom in bt_dicoms:
        synthetic_path = os.path.join(
            processed_study.study_output_dir, dicom, "synth.png"
        )
        ...
        for box in boxes:
            box["coords"] = orient_bounding_box_coords(
                box["coords"], synthetic_pixels.shape, orientation_change
            )
        ...
        if mode == "ground_truth":
            ...
            plotted_img = plot_dicom(
                synthetic_pixels, boxes, gt_df_dicom, pred_color, gt_color, plot_gt=True
            )
        ...
        # plotting prediction boxes only (with NMS algorithm)
            ...
            plotted_img = plot_dicom(
                synthetic_pixels, boxes, pred_color, gt_color, plot_gt=False
            )
