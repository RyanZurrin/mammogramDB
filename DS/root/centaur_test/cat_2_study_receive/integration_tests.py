...
def test_T_161():
    ...
    try:
        ...
        # Create StudiesDB
        studies_db = StudiesDB(
            os.path.join(output_dir, const_deploy.STUDIES_DB_FILE_NAME)
        )
        ...
        for study in studies:
            ...
            assert osp.isdir(
                dest_study_path
            ), f"Study not found in folder {dest_study_path}"
            source_study_path = osp.join(root_folder, study)
            # TODO: ensure the files are identical (MD5 is different)...
            assert len(os.listdir(dest_study_path)) == len(
                os.listdir(source_study_path)
...
def test_T_213():
    ...
    try:
        ...
        studies_db_path = os.path.join(
            studies_db_dir, const_deploy.STUDIES_DB_FILE_NAME
        )
        studies_db = StudiesDB(studies_db_path)

        # Start dir monitor
        input_dir_monitor = DirMonitorInput(
            input_dir,
            studies_db,
        ...
        for study_name in study_names:
            ...
            assert (
                df.empty
            ), f"A row for the study {study_name} should not be inserted by input_dir_monitor"
            ...
            print(
                "Wait until input_dir_monitor inserts a row for the study into StudiesDB..."
            )
            assert (
                input_dir_monitor.check_new_studies() == 0
            ), f"Study {study_name} inserted in StudiesDB before expected"
            time.sleep(NEW_STUDY_THRESHOLD_SECONDS + 4)
            assert (
                input_dir_monitor.check_new_studies() == 1
            ), f"Study {study_name} was expected to be inserted but it doesn't look like that"

            df = studies_db.query_by_input_path(input_path)
            assert (
                len(df) == 1
            ), "A row for the study should be inserted by input_dir_monitor"
    finally:
        shutil.rmtree(temp_folder)
