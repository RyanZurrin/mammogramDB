...
from DS.root.deephealth_utils.data.utils import dh_dcmread
...
def test_T_4_I01():
    ...
    try:
        ...
        with open(logger_file, "r") as f:
            s = f.read()
        assert (
            "This is an info message that should be logged" in s
        ), "Log message not found"
        assert (
            "This is a warning message that should be logged" in s
        ), "Log message not found"
        assert (
            "DO NOT LOG THIS" not in s
        ), "Logger is saving debug messages when only info and above were expected"
        ...
        for flag in ("FAC-200", "SAC-50"):
            assert (
                flag in reasons
            ), "StudyInstanceUID not present, flag {} expected".format(flag)
            assert (
                flag in s
            ), "Flag {} could not be found in logger. Full content: {}".format(flag, s)
    finally:
...
def test_T_4_I02():
    ...
    try:
        ...
        assert reasons == expected_flags, "Expected flags: {}. Got: {}".format(
            expected_flags, reasons
        )

        passed = pp.preprocess(hologic_files_2)
        assert (
            passed
        ), "The study should have passed the validations. Got errors: {}".format(
        ...
        # Basic assertions
        assert len(dicom_df) == len(hologic_files) + len(
            hologic_files_2
        ), "{} files expected in the dicom dataframe. Got {}".format(
            len(hologic_files) + len(hologic_files_2), len(dicom_df)
        )
        ge_df = dicom_df[dicom_df["study"] == DataManager.STUDY_02_GE]
        assert (
            len(ge_df[ge_df["failed_checks"] != ""]) == 0
        ), "There seems to be unexpected failed checks for correct study"

        # Advanced assertions
        for f, flags, expected in (
            (
                "0.dcm",
                ("FAC-20"),
                (
                    "1.2.840.10008.5.1.4.1.1.1.2.1",
                    "Lorad Selenia_1.2.840.10008.5.1.4.1.1.1.2.1",
                ),
            ),
            ("1.dcm", "FAC-21", ("MGG",)),
        ):
            ...
            assert dicom_df.loc[f]["failed_checks"] == "{}".format(
                flags
            ), "Expected failed-checks column value in file {}: '{}'. Got: {}".format(
                f, flags, dicom_df.loc[f]["failed_checks"]
            )
            for flag, expected_val in zip(flags.split("|"), expected):
                # the corresponding flag column has the right value
                assert (
                    dicom_df.loc[f][flag] == expected_val
                ), "Expected column value in file {}: {}. Got: {}".format(
                    f, expected_val, dicom_df.loc[f][flag]
                )
                # Only that corresponding flag column has a value
                for column in [
                    c
                    for c in dicom_df.columns.tolist()
                    if c.startswith("FAC-") and c not in flags
                ]:
                    assert pd.isnull(
                        dicom_df.loc[f][column]
                    ), "Expected null value for file {} and column {}. Got: {}".format(
                        f, column, dicom_df.loc[f][column]
                    )
    finally:
        if osp.isdir(output_folder):
            shutil.rmtree(output_folder)
...
def test_T_4_I03():
    ...
    try:
        ...
        reasons = pp.get_failed_checks()
        assert reasons == ["SAC-70"], "Only SAC-70 flag expected. Got: {}".format(
            reasons
        )
        ...
        # Assertions
        # No FACs
        assert len(dicom_df) == len(hologic_files_2) + len(
            hologic_files
        ), "Total number of dicom rows expected: {}. Got: {}".format(
            len(hologic_files_2) + len(hologic_files), len(dicom_df)
        )
        assert (
            len(dicom_df[dicom_df["failed_checks"] != ""]) == 0
        ), "Some unexpected failures were found in failed_checks column"
        # 2 studies
        assert (
            len(study_df) == 2
        ), "2 rows expected in the study dataframe. Got {}".format(len(study_df))
        # Hologic OK
        assert (
            study_df.loc[hologic_path]["failed_checks"] == ""
        ), "Expected: (). Got: {}".format(study_df.loc[hologic_path]["failed_checks"])
        # GE failed
        assert (
            study_df.loc[hologic_path2]["failed_checks"] == "SAC-70"
        ), "Expected: ('SAC-70',). Got: {}".format(
            study_df.loc[hologic_path2]["failed_checks"]
        )
        # Only corresponding flag columns are present
        assert study_df.columns.tolist() == [
            "failed_checks",
            "SAC-70",
            "passes_checker",
        ], "Expected columns: ['failed_checks', 'SAC-70', 'passes_checker']. Got: {}".format(
            study_df.columns.tolist()
        )
        # Flag column value is correct
        expected_value = "PatientName: ('{0}2', '{0}', '{0}', '{0}')".format(
            patient_name
        )
        assert (
            study_df.loc[hologic_path2]["SAC-70"] == expected_value
        ), "Expected value for column 'SAC-70': {}. Got: {}".format(
            expected_value, study_df.loc[hologic_path2]["SAC-70"]
        )
    finally:
...
def test_T_4_I04():
    ...
    try:
        ...
        # Assertions
        # No FACs
        assert len(dicom_df) == len(hologic_files2) + len(
            hologic_files
        ), "Total number of dicom rows expected: {}. Got: {}".format(
            len(hologic_files2) + len(hologic_files), len(dicom_df)
        )
        assert (
            len(dicom_df[dicom_df["failed_checks"] != ""]) == 0
        ), "Some unexpected failures were found in failed_checks column"
        ...
        # Flag column value is correct
        expected_value = "PatientName: ('{0}', '{0}', '{0}', '{0}2')".format(
            patient_name
        )
        assert (
            study_df.loc[hologic_path2]["SAC-70"] == expected_value
...
def test_T_222():
    ...
    for study in all_studies:
        ...
        if study not in studies_valid:
            assert (
                len(study_sr_path_list) == 0
            ), "Studies that did not pass checker shouldnt have SR output"
            continue
        assert (
            len(study_sr_path_list) == 1
        ), "Studies that passed checker should only have one DH_SaigeQ_SR_XXX file"
        study_sr_path = study_sr_path_list[0]

        study_results_path = os.path.join(
            study_path, const_deploy.CENTAUR_STUDY_DEPLOY_RESULTS_JSON
        )
        sr = dh_dcmread(study_sr_path)
        study_results = StudyDeployResults.from_json(study_results_path)

        assert check_sr_baseline(
            sr, study_results
        ), "mismatching metadata for study {}".format(study)
        assert check_category(
            sr, study_results
        ), "mismatching category for study {}".format(study)
