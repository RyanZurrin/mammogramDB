...
class HashVerifier:
    ...
    def run(self):
        ...
        # create hash json
            ...
            if self.verbose:
                print(
                    f"Create a hash json {self.json_file} for a list of files and directories {self.paths_to_hash}"
                )
            self.create()
    ...
    def create(self):
        ...
        if os.path.exists(self.json_file):
            ...
            assert (
                self.json_backup_dir is not None
            ...
            if self.error_log_file is not None:
                ...
                with open(self.error_log_file, "a") as f:
                    for path, [
                        expected_hash,
                        observed_hash,
                    ] in hash_mismatch_paths.items():
                        f.write(
                            f"{timestamp_str_format}\t{path}\t{expected_hash}\t{observed_hash}\n"
                        )
            else:
                for path, [expected_hash, observed_hash] in hash_mismatch_paths.items():
                    print(
                        f"Code signature verification mismatch: {path}, "
    ...
    def generate_sha256(self, path, create=False):
        ...
        if os.path.isdir(path):
            ...
            if "docker_image" in path:
                assert (
                    self.docker_image_id is not None
                ), "Docker image id should be assigned for generation of sha256"
    ...
    def generate_sha256_docker_image(cls, docker_image_id):
        ...
        assert (
            proc.stderr == ""
        ), f"Unexpected error when running '{command}': {proc.stderr}"
    ...
    def create_conda_versions_txt(cls, path):
        ...
        assert return_code == 0 and os.path.isfile(
            path
        ), f"Unexpected error when running {command}"
        # Set read only permissions
        os.chmod(path, 0o440)
    ...
    def get_cuda_ver(cls):
        ...
        assert r is not None, "Nvidia version not found. Full version info: {}".format(
            version_info
        )
    ...
    def get_os_ver(cls):
        ...
        assert r is not None, "OS version not found. Full version info: {}".format(
            version_info
        )
    ...
    def get_nvidia_driver_ver(cls):
        ...
        # extract version
        r = re.search(
            " \d{1,3}\.\d{1,3}(\.\d{1,3}){0,1} ", version_info, flags=re.MULTILINE
        )
        assert (
            r is not None
        ), "Nvidia Driver version not found. Full version info: {}".format(version_info)
    ...
    def get_python_ver(cls):
        ...
        # extract version
        r = re.search("\d{1,3}\.\d{1,3}\.\d{1,3}", version_info, flags=re.MULTILINE)
        assert r is not None, "Python version not found. Full version info: {}".format(
            version_info
        )
        version = version_info[r.span()[0] : r.span()[1]]
    ...
    def get_miniconda_ver(cls):
        ...
        assert (
            r is not None
        ), "Miniconda version not found. Full version info: {}".format(version_info)
        version = version_info[r.span()[0] : r.span()[1]]
    ...
    def get_nvidia_docker_ver(cls):
        ...
        assert (
            r is not None
        ), "NVIDIA Docker version not found. Full version info: {}".format(version_info)
...
if __name__ == "__main__":
    ...
    args_parser.add_argument(
        ...
        "-j",
        type=str,
        help="Path to a json file that has SHA hash",
        required=True,
    )
    args_parser.add_argument(
        "--error_log_file", "-e", type=str, help="Path to an error log file"
    )
    args_parser.add_argument(
        "--create",
        nargs="+",
        type=str,
        help="list of paths of files and dirs to generate ssh (eg. file1 dir1 file2)",
    )
    args_parser.add_argument(
        "--json_backup_dir",
        "-b",
        type=str,
        help="Path to directory to backup prior json file",
    )
    args_parser.add_argument(
        "--docker_image_id", "-d", type=str, help="docker image id"
    )
    ...
    hash_verifier = HashVerifier(
        args.json_file,
        args.error_log_file,
        json_backup_dir=args.json_backup_dir,
        paths_to_hash=args.create,
