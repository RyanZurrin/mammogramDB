...
class MonitorService:
    ...
    def start_service(self):
        ...
        assert os.path.isfile(
            self.__heartbeat_log_file
        ), f"Heartbeat file not found in {self.__heartbeat_log_file}"
        while True:
    ...
    def report_crash(self):
        ...
        msg = "CRASH: ExecutionID {} detected crash on {}".format(
            self.__execution_id, current_datetime_in_format
        )
...
if __name__ == "__main__":
    ...
    args_parser.add_argument(
        ...
        "-hf",
        type=str,
        help="Path to the heartbeat log file to monitor",
        required=True,
    )
    args_parser.add_argument(
        "--execution_log_file",
        "-ef",
        type=str,
        help="Path to the execution log file to report",
        required=True,
    )
    args_parser.add_argument(
        "--execution_id",
        "-id",
        type=str,
        help="Execution ID of DH-MT Engine container",
        required=True,
    )
    args_parser.add_argument(
        "--timeout", "-t", type=int, help="Report after X seconds", default=600
    )
    args_parser.add_argument(
        "--monitor_frequency", "-fr", type=int, help="Check every X seconds", default=60
    )
    args_parser.add_argument(
        "--startup_time",
        "-s",
        type=int,
        help="time to wait for centaur to initialize",
        default=300,
    )
    args = args_parser.parse_args()
    print(
        "Monitor will check the file {} every {} seconds and will report a crash after {} seconds of inactivity".format(
            args.heartbeat_log_file, args.monitor_frequency, args.timeout
        )
    )
    monitor = MonitorService(
        args.heartbeat_log_file,
        args.execution_log_file,
        args.execution_id,
        timeout_seconds=args.timeout,
        sleep_for_seconds=args.monitor_frequency,
