    ...
    if input_file:
        ...
        if input_file.endswith(".csv"):
            ...
            raise NotImplementedError(
                f"File {input_file} has an unsupported format (expected one of .csv, .pkl, .txt"
            )

        # Add a study directory column to the input DF
        input_df[const.SPLIT_DF_STUDY_DIR_COL_NAME] = input_df[
            const.SPLIT_DF_FPATH_COL_NAME
        ].apply(lambda p: p.split("/")[-2])
    else:
        ...
        for root, dirs, files in os.walk(input_dir):
            ...
            if len(files) > 0:
                file_list = [
                    os.path.join(root, x).replace(input_dir, const.MOUNT_DIR)
                    for x in files
                ]
        ...
        input_df = pd.DataFrame(
            {
                const.SPLIT_DF_FPATH_COL_NAME: all_files,
                const.SPLIT_DF_STUDY_DIR_COL_NAME: all_studies,
            }
        )
...
def split_inputs(input_df, n_splits):
    ...
    """
    study_splits = np.array_split(
        input_df[const.SPLIT_DF_STUDY_DIR_COL_NAME].unique(), n_splits
    )
    split_dfs = [
        input_df[input_df[const.SPLIT_DF_STUDY_DIR_COL_NAME].isin(study_split)].copy()
        for study_split in study_splits
    ]

    # Check that the concatenated split DFs match up to the original input DF.
    assert (
        pd.concat(split_dfs, ignore_index=True)
        .sort_values(by=const.SPLIT_DF_FPATH_COL_NAME)
        .equals(input_df.sort_values(by=const.SPLIT_DF_FPATH_COL_NAME))
    ), "Split DFs do not equal original DF"
