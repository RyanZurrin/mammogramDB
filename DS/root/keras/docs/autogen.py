...
PAGES = [
    ...
    },
    {
        ...
        "classes": [utils.CustomObjectScope, utils.HDF5Matrix, utils.Sequence],
    },
]
...
def post_process_signature(signature):
    ...
    if len(parts) >= 4:
        ...
        if parts[1] == "helpers":
            signature = "keras.helpers." + ".".join(parts[3:])
        if parts[1] == "backend":
...
def class_to_source_link(cls):
    ...
    link = "https://github.com/keras-team/" "keras/blob/master/" + path + "#L" + str(
        line
    )
    return "[[source]](" + link + ")"
