...
def test_temporal_regression():
    ...
    model = Sequential()
    model.add(
        layers.LSTM(y_train.shape[-1], input_shape=(x_train.shape[1], x_train.shape[2]))
    )
    ...
    assert history.history["loss"][-1] < 1.0
...
def test_stacked_lstm_char_prediction():
    ...
    sequence_length = 2
    sentences = [
        alphabet[i : i + sequence_length]
        for i in range(len(alphabet) - sequence_length)
    ]
    next_chars = [
        alphabet[i + sequence_length] for i in range(len(alphabet) - sequence_length)
    ]
...
def test_masked_temporal():
    ...
    history = model.fit(
        x, ys, validation_split=0.05, batch_size=10, verbose=0, epochs=3
    )
