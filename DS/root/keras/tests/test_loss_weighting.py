...
decimal_precision = {"cntk": 2, "theano": 6, "tensorflow": 6}
...
def test_weighted_metrics_with_sample_weight():
    ...
    model = create_sequential_model()
    model.compile(
        loss=loss, optimizer="rmsprop", metrics=[loss], weighted_metrics=[loss]
    )
    ...
    assert_array_almost_equal(
        h["loss"], h["weighted_" + loss_full_name], decimal=decimal
    )
    ...
    h = history.history
    assert_almost_equal(
        h["val_loss"], h["val_weighted_" + loss_full_name], decimal=decimal
    )
...
def test_weighted_metrics_with_no_sample_weight():
    ...
    history = model.fit(
        x_train, y_train, batch_size=batch_size, epochs=epochs // 3, verbose=0
    )
    ...
    h = history.history
    assert_array_almost_equal(
        h["val_loss"], h["val_" + loss_full_name], decimal=decimal
    )
    assert_array_almost_equal(
        h["val_loss"], h["val_weighted_" + loss_full_name], decimal=decimal
    )
...
def test_weighted_metrics_with_weighted_accuracy_metric():
    model = create_sequential_model()
    model.compile(
        loss=loss, optimizer="rmsprop", metrics=["acc"], weighted_metrics=["acc"]
    )
