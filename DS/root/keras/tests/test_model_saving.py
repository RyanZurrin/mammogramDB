...
def test_sequential_model_saving_2():
    ...
    model = load_model(
        fname, custom_objects={"custom_opt": custom_opt, "custom_loss": custom_loss}
    )
    os.remove(fname)
...
def test_saving_lambda_numpy_array_arguments():
    ...
    outputs = Lambda(
        lambda image, mu, std: (image - mu) / std, arguments={"mu": mean, "std": std}
    )(inputs)
    model = Model(inputs, outputs)
    model.compile(loss="mse", optimizer="sgd", metrics=["acc"])
