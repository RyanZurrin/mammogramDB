...
def test_sequential_fit_generator():
    ...
    model.fit_generator(
        data_generator(True), 5, epochs, validation_data=(x_test, y_test)
    )
    model.fit_generator(
        data_generator(True),
        5,
...
def test_sequential(in_tmpdir):
    ...
    model.fit(
        x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=False
    )
    ...
    prediction = model.predict_generator(
        data_generator(x_test, y_test), 1, max_queue_size=2, verbose=1
    )
    gen_loss = model.evaluate_generator(
        data_generator(x_test, y_test, 50), 1, max_queue_size=2
    )
    pred_loss = K.eval(
        K.mean(losses.get(model.loss)(K.variable(y_test), K.variable(prediction)))
    )
