...
pytestmark = pytest.mark.skipif(
    ...
    and os.environ.get("APP_CHANGED", "True") == "False",
...
NASNET_LIST = [(applications.NASNetMobile, 1056), (applications.NASNetLarge, 4032)]
...
def _test_app_pooling(app, last_dim):
    output_shape = _get_output_shape(
        lambda: app(
            weights=None, include_top=False, pooling=random.choice(["avg", "max"])
        )
    )
    assert output_shape == (None, last_dim)
...
@pytest.mark.skipif(
    (K.backend() != "tensorflow"), reason="NASNets are supported only on TensorFlow"
)
def test_nasnet():
