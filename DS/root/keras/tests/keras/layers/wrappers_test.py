...
def test_TimeDistributed():
    ...
    model.add(
        wrappers.TimeDistributed(
            layers.Embedding(5, 6), batch_input_shape=(10, 3, 4), dtype="int32"
        )
    )
    model.compile(optimizer="rmsprop", loss="mse")
    model.fit(
        np.random.randint(5, size=(10, 3, 4), dtype="int32"),
    ...
    reference.add(
        wrappers.TimeDistributed(
            layers.Embedding(5, 6), input_shape=(3, 4), dtype="int32"
        )
    )
    reference.compile(optimizer="rmsprop", loss="mse")
    reference.layers[0].set_weights(weights)
    ...
    model.add(
        wrappers.TimeDistributed(
            layers.Conv2D(5, (2, 2), padding="same"), input_shape=(2, 4, 4, 3)
        )
    )
    model.add(layers.Activation("relu"))
    model.compile(optimizer="rmsprop", loss="mse")
...
@keras_test
@pytest.mark.skipif((K.backend() == "cntk"), reason="Flaky with CNTK backend")
def test_TimeDistributed_learning_phase():
    # test layers that need learning_phase to be set
    np.random.seed(1234)
    x = Input(shape=(3, 2))
    y = wrappers.TimeDistributed(layers.Dropout(0.999))(x, training=True)
    model = Model(x, y)
    y = model.predict(np.random.random((10, 3, 2)))
    assert_allclose(np.mean(y), 0.0, atol=1e-1, rtol=1e-1)
    ...
    for mode in ["sum", "concat"]:
        ...
        model.add(
            wrappers.Bidirectional(
                rnn(output_dim, dropout=dropout_rate, recurrent_dropout=dropout_rate),
                merge_mode=mode,
        ...
        outputs = wrappers.Bidirectional(
            rnn(output_dim, dropout=dropout_rate, recurrent_dropout=dropout_rate),
            merge_mode=mode,
        )(inputs)
...
def test_Bidirectional_dropout(merge_mode):
    ...
    inputs = Input((timesteps, dim))
    wrapped = wrappers.Bidirectional(
