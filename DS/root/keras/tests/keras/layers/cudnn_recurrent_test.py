...
@keras_test
@pytest.mark.skipif(
    (keras.backend.backend() != "tensorflow"), reason="Requires TensorFlow backend"
)
@pytest.mark.skipif(
    not keras.backend.tensorflow_backend._get_available_gpus(), reason="Requires GPU"
)
...
def test_load_weights_between_noncudnn_rnn(
    rnn_type, to_cudnn, bidirectional, implementation
):
    input_size = 10
    timesteps = 6
    input_shape = (timesteps, input_size)
    ...
    def convert_weights(source_layer, target_layer):
        ...
        weights = keras.engine.topology.preprocess_weights_for_loading(
...
def test_preprocess_weights_for_loading_gru_incompatible():
    ...
    assert_not_compatible(
        gru(), gru(cudnn=True), "GRU(reset_after=False) is not compatible with CuDNNGRU"
    )
    assert_not_compatible(
        gru(cudnn=True), gru(), "CuDNNGRU is not compatible with GRU(reset_after=False)"
    )
    assert_not_compatible(
        gru(),
