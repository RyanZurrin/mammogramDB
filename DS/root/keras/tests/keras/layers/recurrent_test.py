...
def rnn_test(f):
    ...
    return pytest.mark.parametrize(
        "layer_class", [recurrent.SimpleRNN, recurrent.GRU, recurrent.LSTM]
    )(f)
...
def rnn_cell_test(f):
    f = keras_test(f)
    return pytest.mark.parametrize(
        "cell_class", [recurrent.SimpleRNNCell, recurrent.GRUCell, recurrent.LSTMCell]
    )(f)
...
def test_stateful_invalid_use(layer_class):
    layer = layer_class(
        units, stateful=True, batch_input_shape=(num_samples, timesteps, embedding_dim)
    )
    model = Sequential()
    model.add(layer)
    model.compile("sgd", "mse")
...
@rnn_test
@pytest.mark.skipif((K.backend() in ["theano"]), reason="Not supported.")
...
def test_implementation_mode(layer_class):
    for mode in [1, 2]:
        ...
        layer_test(
            layer_class,
            kwargs={"units": units, "implementation": mode, "use_bias": False},
            input_shape=(num_samples, timesteps, embedding_dim),
        )
...
def test_specify_initial_state_non_keras_tensor(layer_class):
    ...
    inputs = Input((timesteps, embedding_dim))
    initial_state = [
        K.random_normal_variable((num_samples, units), 0, 1) for _ in range(num_states)
    ]
    layer = layer_class(units)
    output = layer(inputs, initial_state=initial_state)
...
def test_reset_states_with_values(layer_class):
    ...
    np.testing.assert_allclose(
        K.eval(layer.states[0]), np.zeros(K.int_shape(layer.states[0])), atol=1e-4
    )
    ...
    np.testing.assert_allclose(
        K.eval(layer.states[0]), np.ones(K.int_shape(layer.states[0])), atol=1e-4
    )
...
def test_minimal_rnn_cell_non_layer():
    class MinimalRNNCell(object):
        def __init__(self, units, input_dim):
            ...
            self.kernel = keras.backend.variable(np.random.random((input_dim, units)))
...
@keras_test
@pytest.mark.skipif((K.backend() in ["cntk", "theano"]), reason="Not supported.")
...
def test_rnn_cell_with_constants_layer_passing_initial_state():
    ...
    model.compile(optimizer="rmsprop", loss="mse")
    model.train_on_batch(
        [np.zeros((6, 5, 5)), np.zeros((6, 32)), np.zeros((6, 3))], np.zeros((6, 32))
    )
