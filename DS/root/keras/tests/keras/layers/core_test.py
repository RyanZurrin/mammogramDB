...
def test_dropout():
    ...
    layer_test(
        layers.Dropout, kwargs={"rate": 0.5, "noise_shape": [3, 1]}, input_shape=(3, 2)
    )
    ...
    for data_format in ["channels_last", "channels_first"]:
        for shape in [(4, 5), (4, 5, 6)]:
            ...
            layer_test(
                layers.SpatialDropout2D if len(shape) == 2 else layers.SpatialDropout3D,
                kwargs={"rate": 0.5, "data_format": data_format},
            ...
            with pytest.raises(ValueError):
                layer_test(
                    ...
                    if len(shape) == 2
                    else layers.SpatialDropout3D,
                    kwargs={"rate": 0.5, "data_format": "channels_middle"},
                    input_shape=input_shape,
                )
...
@keras_test
def test_reshape():
    layer_test(layers.Reshape, kwargs={"target_shape": (8, 1)}, input_shape=(3, 2, 4))

    layer_test(layers.Reshape, kwargs={"target_shape": (-1, 1)}, input_shape=(3, 2, 4))

    layer_test(layers.Reshape, kwargs={"target_shape": (1, -1)}, input_shape=(3, 2, 4))

    layer_test(
        layers.Reshape, kwargs={"target_shape": (-1, 1)}, input_shape=(None, None, 4)
    )
...
@keras_test
def test_flatten():
    layer_test(layers.Flatten, kwargs={}, input_shape=(3, 2, 4))
...
def test_lambda():
    ...
    layer_test(
        layers.Lambda,
        kwargs={"function": antirectifier, "output_shape": antirectifier_output_shape},
        input_shape=(3, 2),
    )
...
def test_dense():
    ...
    layer = layers.Dense(
        3, kernel_regularizer=regularizers.l1(0.01), bias_regularizer="l1"
    )
    layer.build((None, 4))
