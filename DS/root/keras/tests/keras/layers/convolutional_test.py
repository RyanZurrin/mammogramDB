...
@keras_test
@pytest.mark.skipif(
    (K.backend() == "cntk"), reason="cntk does not support dilated conv"
)
...
def test_globalpooling_1d():
    layer_test(pooling.GlobalMaxPooling1D, input_shape=(3, 4, 5))
    layer_test(pooling.GlobalAveragePooling1D, input_shape=(3, 4, 5))
...
def test_zero_padding_1d():
    ...
    # basic test
    layer_test(
        convolutional.ZeroPadding1D, kwargs={"padding": 2}, input_shape=inputs.shape
    )
    layer_test(
    ...
    for offset in [0, 1, -1, -2]:
        assert_allclose(np_output[:, offset, :], 0.0)
    assert_allclose(np_output[:, 2:-2, :], 1.0)
    ...
    for left_offset in [0]:
        assert_allclose(np_output[:, left_offset, :], 0.0)
    for right_offset in [-1, -2]:
        assert_allclose(np_output[:, right_offset, :], 0.0)
    assert_allclose(np_output[:, 1:-2, :], 1.0)
    layer.get_config()
    ...
    for data_format in ["channels_first", "channels_last"]:
        ...
        if data_format == "channels_last":
            for offset in [0, 1, -1, -2]:
                assert_allclose(np_output[:, offset, :, :], 0.0)
                assert_allclose(np_output[:, :, offset, :], 0.0)
            assert_allclose(np_output[:, 2:-2, 2:-2, :], 1.0)
        ...
        layer = convolutional.ZeroPadding2D(
        ...
        if data_format == "channels_last":
            for top_offset in [0]:
                assert_allclose(np_output[:, top_offset, :, :], 0.0)
            for bottom_offset in [-1, -2]:
                assert_allclose(np_output[:, bottom_offset, :, :], 0.0)
            for left_offset in [0, 1, 2]:
                assert_allclose(np_output[:, :, left_offset, :], 0.0)
            for right_offset in [-1, -2, -3, -4]:
                assert_allclose(np_output[:, :, right_offset, :], 0.0)
            assert_allclose(np_output[:, 1:-2, 3:-4, :], 1.0)
        elif data_format == "channels_first":
            for top_offset in [0]:
                assert_allclose(np_output[:, :, top_offset, :], 0.0)
            for bottom_offset in [-1, -2]:
    ...
    for data_format in ["channels_first", "channels_last"]:
        ...
        if data_format == "channels_last":
            for offset in [0, 1, -1, -2]:
                ...
                assert_allclose(np_output[:, :, offset, :, :], 0.0)
                assert_allclose(np_output[:, :, :, offset, :], 0.0)
            assert_allclose(np_output[:, 2:-2, 2:-2, 2:-2, :], 1.0)
        elif data_format == "channels_first":
            for offset in [0, 1, -1, -2]:
                assert_allclose(np_output[:, :, offset, :, :], 0.0)
                assert_allclose(np_output[:, :, :, offset, :], 0.0)
        ...
        layer = convolutional.ZeroPadding3D(
        ...
        if data_format == "channels_last":
            for dim1_offset in [0, -1, -2]:
                assert_allclose(np_output[:, dim1_offset, :, :, :], 0.0)
            for dim2_offset in [0, 1, 2, -1, -2, -3, -4]:
                assert_allclose(np_output[:, :, dim2_offset, :, :], 0.0)
            for dim3_offset in [-1, -2]:
                assert_allclose(np_output[:, :, :, dim3_offset, :], 0.0)
            assert_allclose(np_output[:, 1:-2, 3:-4, 0:-2, :], 1.0)
        elif data_format == "channels_first":
            for dim1_offset in [0, -1, -2]:
                assert_allclose(np_output[:, :, dim1_offset, :, :], 0.0)
            for dim2_offset in [0, 1, 2, -1, -2, -3, -4]:
...
@pytest.mark.skipif((K.backend() == "cntk"), reason="cntk does not support it yet")
def test_upsampling_3d():
    ...
    for data_format in ["channels_first", "channels_last"]:
        if data_format == "channels_first":
            inputs = np.random.rand(
                num_samples, stack_size, input_len_dim1, input_len_dim2, input_len_dim3
            )
        else:  # tf
            inputs = np.random.rand(
                num_samples, input_len_dim1, input_len_dim2, input_len_dim3, stack_size
...
@keras_test
@pytest.mark.skipif(
    (K.backend() == "cntk"), reason="cntk does not support slice to 0 dimension"
)
def test_cropping_1d():
    ...
    layer_test(
        convolutional.Cropping1D, kwargs={"cropping": (2, 2)}, input_shape=inputs.shape
    )
...
def test_cropping_2d():
    ...
    for data_format in ["channels_first", "channels_last"]:
        ...
        if data_format == "channels_first":
            expected_out = inputs[
                :, :, cropping[0][0] : -cropping[0][1], cropping[1][0] : -cropping[1][1]
            ]
        else:
            expected_out = inputs[
                :, cropping[0][0] : -cropping[0][1], cropping[1][0] : -cropping[1][1], :
