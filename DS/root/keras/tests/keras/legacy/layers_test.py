...
@keras_test
def test_highway():
    layer_test(legacy_layers.Highway, kwargs={}, input_shape=(3, 2))

    layer_test(
        legacy_layers.Highway,
        kwargs={
...
def test_maxout_dense():
    layer_test(legacy_layers.MaxoutDense, kwargs={"output_dim": 3}, input_shape=(3, 2))

    layer_test(
        legacy_layers.MaxoutDense,
        kwargs={
            "output_dim": 3,
            "W_regularizer": regularizers.l2(0.01),
            "b_regularizer": regularizers.l1(0.01),
...
def test_merge():
    ...
    input_b = layers.Input(shape=input_shapes[1][1:])
    merged = legacy_layers.merge(
        [input_a, input_b], mode=fn_mode, output_shape=fn_output_shape
    )
    model = models.Model([input_a, input_b], merged)
    expected_output_shape = model.compute_output_shape(input_shapes)
    actual_output_shape = model.predict(inputs).shape
    ...
    merged = legacy_layers.merge(
        [a, b], mode=fn_mode, output_shape=fn_output_shape, output_mask=fn_output_mask
    )
    model = models.Model([input_a, input_b], merged)
    expected_output_shape = model.compute_output_shape(input_shapes)
    actual_output_shape = model.predict(inputs).shape
...
def test_merge_mask_2d():
    ...
    merged_concat = legacy_layers.merge(
        [masked_a, masked_b], mode="concat", concat_axis=1
    )
    merged_concat_mixed = legacy_layers.merge(
        [masked_a, input_b], mode="concat", concat_axis=1
    )
...
def test_sequential_regression():
    ...
    branch_1.add(
        layers.Embedding(input_dim=100, output_dim=10, input_length=2, name="embed_1")
    )
    ...
    branch_1_2 = models.Sequential(
        [legacy_layers.Merge([branch_1, branch_2], mode="concat")], name="branch_1_2"
    )
    ...
    model = models.Sequential(
        [legacy_layers.Merge([branch_1_2, branch_3], mode="concat")], name="final"
    )
