...
def test_dense_legacy_interface():
    ...
    old_layer = keras.layers.Dense(
        2, bias=True, b_regularizer="l1", b_constraint="maxnorm", name="d"
    )
    new_layer = keras.layers.Dense(
        2, use_bias=True, bias_regularizer="l1", bias_constraint="max_norm", name="d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_embedding_legacy_interface():
    ...
    old_layer = keras.layers.Embedding(
        input_dim=4,
        output_dim=2,
        name="d",
        init="normal",
    ...
    new_layer = keras.layers.Embedding(
        input_dim=4,
        output_dim=2,
        name="d",
...
def test_maxpooling1d_legacy_interface():
    old_layer = keras.layers.MaxPool1D(
        pool_length=2, border_mode="valid", name="maxpool1d"
    )
...
def test_lstm_legacy_interface():
    ...
    old_layer = keras.layers.LSTM(
        input_shape=[3, 5], output_dim=2, name="d", consume_less="mem"
    )
    new_layer = keras.layers.LSTM(2, input_shape=[3, 5], name="d", implementation=1)
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.LSTM(
        input_dim=5, input_length=3, output_dim=2, name="d", consume_less="mem"
    )
    new_layer = keras.layers.LSTM(2, input_shape=[3, 5], name="d", implementation=1)
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.LSTM(
        input_dim=5, output_dim=2, name="d", consume_less="mem"
    )
    new_layer = keras.layers.LSTM(2, input_shape=[None, 5], name="d", implementation=1)
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.LSTM(
        input_shape=[3, 5], output_dim=2, name="d", consume_less="gpu"
    )
...
def test_simplernn_legacy_interface():
    ...
    new_layer = keras.layers.SimpleRNN(
        2,
        kernel_initializer="normal",
        recurrent_initializer="glorot_uniform",
        kernel_regularizer="l1",
...
def test_maxpooling2d_legacy_interface():
    ...
    new_layer = keras.layers.MaxPool2D(
        pool_size=2, padding="valid", data_format="channels_first", name="maxpool2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_avgpooling3d_legacy_interface():
    ...
    new_layer = keras.layers.AvgPool3D(
        pool_size=(2, 2, 2), strides=(2, 2, 2), padding="valid", name="avgpooling3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_global_maxpooling2d_legacy_interface():
    old_layer = keras.layers.GlobalMaxPooling2D(
        dim_ordering="tf", name="global_maxpool2d"
    )
    new_layer = keras.layers.GlobalMaxPool2D(
        data_format="channels_last", name="global_maxpool2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalMaxPooling2D(
        dim_ordering="th", name="global_maxpool2d"
    )
    new_layer = keras.layers.GlobalMaxPool2D(
        data_format="channels_first", name="global_maxpool2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalMaxPooling2D(
        dim_ordering="default", name="global_maxpool2d"
    )
...
def test_global_avgpooling2d_legacy_interface():
    old_layer = keras.layers.GlobalAveragePooling2D(
        dim_ordering="tf", name="global_avgpool2d"
    )
    new_layer = keras.layers.GlobalAvgPool2D(
        data_format="channels_last", name="global_avgpool2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalAveragePooling2D(
        dim_ordering="th", name="global_avgpool2d"
    )
    new_layer = keras.layers.GlobalAvgPool2D(
        data_format="channels_first", name="global_avgpool2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalAveragePooling2D(
        dim_ordering="default", name="global_avgpool2d"
    )
...
def test_global_maxpooling3d_legacy_interface():
    old_layer = keras.layers.GlobalMaxPooling3D(
        dim_ordering="tf", name="global_maxpool3d"
    )
    new_layer = keras.layers.GlobalMaxPool3D(
        data_format="channels_last", name="global_maxpool3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalMaxPooling3D(
        dim_ordering="th", name="global_maxpool3d"
    )
    new_layer = keras.layers.GlobalMaxPool3D(
        data_format="channels_first", name="global_maxpool3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalMaxPooling3D(
        dim_ordering="default", name="global_maxpool3d"
    )
...
def test_global_avgpooling3d_legacy_interface():
    old_layer = keras.layers.GlobalAveragePooling3D(
        dim_ordering="tf", name="global_avgpool3d"
    )
    new_layer = keras.layers.GlobalAvgPool3D(
        data_format="channels_last", name="global_avgpool3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalAveragePooling3D(
        dim_ordering="th", name="global_avgpool3d"
    )
    new_layer = keras.layers.GlobalAvgPool3D(
        data_format="channels_first", name="global_avgpool3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.GlobalAveragePooling3D(
        dim_ordering="default", name="global_avgpool3d"
    )
...
def test_upsampling2d_legacy_interface():
    ...
    new_layer = keras.layers.UpSampling2D(
        (2, 2), data_format="channels_last", name="us2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_upsampling3d_legacy_interface():
    ...
    new_layer = keras.layers.UpSampling3D(
        (2, 2, 2), data_format="channels_last", name="us3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_conv2d_legacy_interface():
    ...
    old_layer = keras.layers.Convolution2D(
        5,
        3,
        3,
        init="normal",
...
def test_deconv2d_legacy_interface():
    ...
    old_layer = keras.layers.Deconvolution2D(
        5, 3, 3, output_shape=(6, 7, 5), name="deconv"
    )
    new_layer = keras.layers.Conv2DTranspose(5, (3, 3), name="deconv")
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.Deconvolution2D(
        5, 3, nb_col=3, output_shape=(6, 7, 5), name="deconv"
    )
    new_layer = keras.layers.Conv2DTranspose(5, (3, 3), name="deconv")
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.Deconvolution2D(
        5, nb_row=3, nb_col=3, output_shape=(6, 7, 5), name="deconv"
    )
    new_layer = keras.layers.Conv2DTranspose(5, (3, 3), name="deconv")
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_conv1d_legacy_interface():
    old_layer = keras.layers.Convolution1D(
        5, filter_length=3, input_dim=3, input_length=4, name="conv"
    )
...
def test_separable_conv2d_legacy_interface():
    ...
    new_layer = keras.layers.SeparableConv2D(
        5,
        (3, 3),
...
def test_conv3d_legacy_interface():
    ...
    old_layer = keras.layers.Convolution3D(
        5, 3, kernel_dim2=3, kernel_dim3=4, name="conv"
    )
    new_layer = keras.layers.Conv3D(5, (3, 3, 4), name="conv")
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())

    old_layer = keras.layers.Convolution3D(
        5, kernel_dim1=3, kernel_dim2=3, kernel_dim3=4, name="conv"
    )
    new_layer = keras.layers.Conv3D(5, (3, 3, 4), name="conv")
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_convlstm2d_legacy_interface():
    ...
    old_layer = keras.layers.ConvLSTM2D(
        5,
        3,
        3,
        init="normal",
    ...
    new_layer = keras.layers.ConvLSTM2D(
        5,
        (3, 3),
        kernel_initializer="normal",
...
def test_batchnorm_legacy_interface():
    ...
    old_layer = keras.layers.BatchNormalization(
        mode=0, beta_init="one", gamma_init="uniform", name="bn"
    )
    new_layer = keras.layers.BatchNormalization(
        beta_initializer="ones", gamma_initializer="uniform", name="bn"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_atrousconv1d_legacy_interface():
    old_layer = keras.layers.AtrousConvolution1D(
        5,
        3,
        init="normal",
        subsample_length=2,
...
def test_zeropadding2d_legacy_interface():
    old_layer = keras.layers.ZeroPadding2D(
        padding={"right_pad": 4, "bottom_pad": 2, "top_pad": 1, "left_pad": 3},
        dim_ordering="tf",
        name="zp2d",
    )
    new_layer = keras.layers.ZeroPadding2D(
        ((1, 2), (3, 4)), data_format="channels_last", name="zp2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_zeropadding3d_legacy_interface():
    ...
    new_layer = keras.layers.ZeroPadding3D(
        (2, 2, 2), data_format="channels_last", name="zp3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer.get_config())
...
def test_spatialdropout2d_legacy_interface():
    old_layer = keras.layers.SpatialDropout2D(p=0.5, dim_ordering="tf", name="sd2d")
    new_layer_1 = keras.layers.SpatialDropout2D(
        rate=0.5, data_format="channels_last", name="sd2d"
    )
    new_layer_2 = keras.layers.SpatialDropout2D(
        0.5, data_format="channels_last", name="sd2d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer_1.get_config())
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer_2.get_config())
...
def test_spatialdropout3d_legacy_interface():
    old_layer = keras.layers.SpatialDropout3D(p=0.5, dim_ordering="tf", name="sd3d")
    new_layer_1 = keras.layers.SpatialDropout3D(
        rate=0.5, data_format="channels_last", name="sd3d"
    )
    new_layer_2 = keras.layers.SpatialDropout3D(
        0.5, data_format="channels_last", name="sd3d"
    )
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer_1.get_config())
    assert json.dumps(old_layer.get_config()) == json.dumps(new_layer_2.get_config())
