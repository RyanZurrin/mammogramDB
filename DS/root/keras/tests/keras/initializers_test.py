...
def test_uniform(tensor_shape):
    _runner(
        initializers.RandomUniform(minval=-1, maxval=1),
        tensor_shape,
        target_mean=0.0,
        target_max=1,
        target_min=-1,
...
def test_lecun_normal(tensor_shape):
    ...
    _runner(
        initializers.lecun_normal(), tensor_shape, target_mean=0.0, target_std=scale
    )


@pytest.mark.parametrize("tensor_shape", [FC_SHAPE, CONV_SHAPE], ids=["FC", "CONV"])
def test_orthogonal(tensor_shape):
    _runner(initializers.orthogonal(), tensor_shape, target_mean=0.0)
...
def test_zero(tensor_shape):
    _runner(initializers.zeros(), tensor_shape, target_mean=0.0, target_max=0.0)


@pytest.mark.parametrize("tensor_shape", [FC_SHAPE, CONV_SHAPE], ids=["FC", "CONV"])
def test_one(tensor_shape):
    _runner(initializers.ones(), tensor_shape, target_mean=1.0, target_max=1.0)
