...
def test_valid_compute_mask():
    ...
    assert model.layers[0].compute_mask([model.input], [0.0, 1.0]) == [0.0, 1.0]
...
def test_multi_input_layer():
    ...
    assert model.compute_output_shape([(None, 32), (None, 32)]) == [
        (None, 64),
        (None, 5),
    ]
    ...
    assert [l.name for l in model.layers][2:] == [
        "dense_1",
        "merge",
        "dense_2",
        "dense_3",
    ...
    assert [l.name for l in recreated_model.layers][2:] == [
        "dense_1",
        "merge",
        "dense_2",
        "dense_3",
...
def test_load_layers():
    ...
    for j in range(2):  # bidirectional
        for i in range(4):
            ...
            weight_tensor_bi_convlstm_old.append(
                np.zeros((3, 3, 10, 10))
            )  # recurrent kernel
    ...
    weight_tensor_bi_convlstm_new = topology.preprocess_weights_for_loading(
        bi_convlstm_layer, weight_tensor_bi_convlstm_old, original_keras_version="1"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        layers.Conv2DTranspose(
            2, (5, 5), input_shape=[7, 7, 3], data_format="channels_first"
        ),
    ],
...
@pytest.mark.skipif(
    not K.tensorflow_backend._get_available_gpus(), reason="Requires GPU"
)
def test_preprocess_weights_for_loading_cudnn_rnn_should_be_idempotent(
    layer_class, layer_args
):
...
def test_multi_output_mask():
    ...
    class ArbitraryMultiOutputLayer(Layer):
        ...
        def compute_output_shape(self, input_shape):
            out_shape = super(ArbitraryMultiOutputLayer, self).compute_output_shape(
                input_shape
            )
