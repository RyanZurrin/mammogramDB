...
def test_model_methods():
    ...
    model.compile(
        optimizer, loss, metrics=[], loss_weights=loss_weights, sample_weight_mode=None
    )

    # test train_on_batch
    out = model.train_on_batch([input_a_np, input_b_np], [output_a_np, output_b_np])
    out = model.train_on_batch(
        {"input_a": input_a_np, "input_b": input_b_np}, [output_a_np, output_b_np]
    )
    out = model.train_on_batch(
    ...
    # test fit
    out = model.fit(
        [input_a_np, input_b_np], [output_a_np, output_b_np], epochs=1, batch_size=4
    )
    out = model.fit(
        {"input_a": input_a_np, "input_b": input_b_np},
    ...
    out = model.evaluate(
        [input_a_np, input_b_np], [output_a_np, output_b_np], batch_size=4
    )
    ...
    # and this as well
    model.compile(
        optimizer, loss, metrics={"dense_1": ["acc"]}, sample_weight_mode=None
    )
    ...
    tracker_cb = LambdaCallback(
        on_epoch_begin=on_epoch_begin, on_batch_begin=on_batch_begin
    )
    ...
    def gen_data(batch_sz):
        while True:
            yield (
                [np.random.random((batch_sz, 3)), np.random.random((batch_sz, 3))],
    ...
    out = model.fit(
        [input_a_np, input_b_np], [output_a_np, output_b_np], batch_size=4, epochs=1
    )
    out = model.evaluate(
        [input_a_np, input_b_np], [output_a_np, output_b_np], batch_size=4
    )
    ...
    def gen_data(i):
        while True:
            gen_counters[i] += 1
            yield (
                [np.random.random((1, 3)), np.random.random((1, 3))],
                [np.random.random((1, 4)), np.random.random((1, 3))],
            )

    out = model.fit_generator(
        generator=gen_data(0),
        epochs=3,
        steps_per_epoch=2,
...
def test_warnings():
    ...
    with pytest.warns(Warning) as w:
        out = model.fit_generator(
            gen_data(4), steps_per_epoch=10, use_multiprocessing=True, workers=2
        )
    ...
    with pytest.warns(None) as w:
        out = model.fit_generator(
            RandomSequence(3), steps_per_epoch=4, use_multiprocessing=True, workers=2
        )
    assert all(
        ["Sequence" not in str(w_.message) for w_ in w]
...
@pytest.mark.skipif(
    K.backend() != "tensorflow", reason="sparse operations supported only by TensorFlow"
)
@keras_test
...
def test_model_with_input_feed_tensor():
    ...
    # test fit
    out = model.fit(
        {"input_b": input_b_np}, [output_a_np, output_b_np], epochs=1, batch_size=10
    )
...
@keras_test
@pytest.mark.skipif(
    (K.backend() == "cntk"), reason="cntk does not support external loss yet"
)
def test_model_with_external_loss():
    ...
    if K.backend() == "tensorflow":
        ...
        with pytest.raises(ValueError):
            out = model.fit(
                None, None, epochs=1, steps_per_epoch=None, validation_steps=2
            )
...
def test_model_custom_target_tensors():
    ...
    out = model.train_on_batch(
        [input_a_np, input_b_np],
        [output_a_np, output_b_np],
        {y: np.random.random((10, 4)), y1: np.random.random((10, 3))},
    )
