...
def cntk_func_three_tensor(function_name, x_shape, y, z, **kwargs):
    ...
    output_cntk = getattr(KC, function_name)(
        xc, KC.variable(y), KC.variable(z), **kwargs
    )
    return KC.function([xc], [output_cntk])
...
@keras_test
def check_single_tensor_operation(
    function_name, x_shape_or_val, backend_list, **kwargs
):
    ...
    for k in backend_list:
        if shape_or_val:
            if (k == KC) & (cntk_dynamicity):
                z = cntk_func_single_tensor(function_name, x_shape, **kwargs)([x_val])[
                    0
                ]
            else:
...
@keras_test
def check_two_tensor_operation(
    function_name, x_shape_or_val, y_shape_or_val, backend_list, **kwargs
):
    shape_or_val = kwargs.pop("shape_or_val", True)
    ...
    for k in backend_list:
        if shape_or_val:
            if (k == KC) & (cntk_dynamicity):
                z = cntk_func_two_tensor(function_name, x_shape, y=y_val, **kwargs)(
                    [x_val]
                )[0]
            elif (k == KC) & (cntk_two_dynamicity):
                z = cntk_func_two_tensor(function_name, x_shape, y=y_shape, **kwargs)(
                    [x_val, y_val]
                )[0]
            elif (k == KTH) & (function_name[:4] == "conv"):
                z = k.eval(
                    getattr(k, function_name)(
...
class TestBackend(object):
    ...
    def test_linear_operations(self):
        ...
        check_two_tensor_operation(
            "batch_dot", (32, 20), (32, 20), BACKENDS, cntk_two_dynamicity=True, axes=1
        )
        check_two_tensor_operation(
            "batch_dot",
            (32, 20),
    ...
    def test_gather(self):
        ...
        inds = [1, 3, 7, 9]
        z_list = [
            k.eval(k.gather(k.variable(ref), k.variable(inds, dtype="int32")))
            for k in BACKENDS
        ]
    ...
    def test_value_manipulation(self):
        ...
        for function_name in [
            ...
            v_list = [getattr(k, function_name)(k.variable(val)) for k in BACKENDS]
    ...
    def test_elementwise_operations(self):
        ...
        check_single_tensor_operation(
            "mean", (4, 2, 3), BACKENDS, axis=-1, keepdims=True
        )
        ...
        check_single_tensor_operation(
            "clip", (4, 2), BACKENDS, min_value=0.4, max_value=0.6
        )
    ...
    def test_function_tf_fetches(self):
        ...
        f = KTF.function(
            ...
            fetches=[KTF.update(y, 5.0)],
        )
        output = f([10.0, 20.0])
        assert output == [30.0]
        assert KTF.get_session().run(fetches=[x, y]) == [11.0, 5.0]
    ...
    def test_function_tf_feed_dict(self):
        ...
        f = KTF.function(
            ...
            feed_dict=feed_dict,
            fetches=[KTF.update(y, y_placeholder * 10.0)],
        )
        output = f([10.0])
        assert output == [11.0]
        assert KTF.get_session().run(fetches=[x, y]) == [20.0, 30.0]

        # updated value in feed_dict will be modified within the K.function()
        feed_dict[y_placeholder] = 4.0
        output = f([20.0])
        assert output == [21.0]
        assert KTF.get_session().run(fetches=[x, y]) == [30.0, 40.0]
    ...
    def test_rnn_no_states(self):
        ...
        for k in BACKENDS:
            ...
            initial_states = []
            last_output, outputs, new_states = k.rnn(
                rnn_fn, inputs, initial_states, go_backwards=False, mask=None
            )
    ...
    def test_logsumexp(self, x_np, axis, keepdims):
        ...
        for k in BACKENDS:
            x = k.variable(x_np)
            assert_allclose(
                k.eval(k.logsumexp(x, axis=axis, keepdims=keepdims)),
    ...
    def test_logsumexp_optim(self):
        ...
        """
        for k in [KTF]:
            x_np = np.array([1e4, 1e-4])
            assert_allclose(
                k.eval(k.logsumexp(k.variable(x_np), axis=0)), 1e4, rtol=1e-5
            )
    ...
    def test_dropout(self):
        ...
        z_list = [k.eval(k.dropout(k.variable(val), level=0.2)) for k in BACKENDS]
        ...
        z_list = [
            k.eval(k.dropout(k.variable(val), level=0.2, noise_shape=list(val.shape)))
            for k in BACKENDS
        ]
        assert_list_pairwise(z_list, allclose=False)
        # dropout patterns are different, only check mean
    ...
    def test_nn_operations(self):
        check_single_tensor_operation(
            "relu", (4, 2), BACKENDS, alpha=0.1, max_value=0.5
        )
        ...
        check_two_tensor_operation(
            "binary_crossentropy", (4, 2), (4, 2), BACKENDS, from_logits=True
        )
        ...
        check_two_tensor_operation(
            "categorical_crossentropy", (4, 2), (4, 2), [KTH, KTF], from_logits=True
        )
        xval = np.asarray(
            [
                [0.26157712, 0.0432167],
                [-0.43380741, 0.30559841],
                [0.20225059, -0.38956559],
                [-0.13805378, 0.08506755],
            ],
            dtype=np.float32,
        )
        yval = np.asarray(
            [
                [0.46221867, 0.53778133],
                [0.51228984, 0.48771016],
                [0.64916514, 0.35083486],
                [0.47028078, 0.52971922],
            ],
            dtype=np.float32,
        )
        check_two_tensor_operation(
        ...
        check_two_tensor_operation(
            "binary_crossentropy", (4, 2), (4, 2), BACKENDS, from_logits=False
        )
        check_two_tensor_operation(
            "categorical_crossentropy", (4, 2), (4, 2), BACKENDS, from_logits=False
        )
    ...
    def test_in_top_k(self):
        ...
        for k in range(num_classes + 1):
            z_list = [
                b.eval(
                    b.in_top_k(
                        b.variable(predictions, dtype="float32"),
                        b.variable(targets, dtype="int32"),
                        k,
                    )
                )
                for b in [KTH, KTF]
        ...
        for i in range(batch_size):
            idx_identical = np.random.choice(
                num_classes, size=num_identical, replace=False
            )
        ...
        for input_shape, data_format in [
            ...
            for kernel_shape in [(2, 2), (4, 3)]:
                for depth_multiplier in [1, 2]:
                    _, depthwise_val = parse_shape_or_val(
                        kernel_shape + (input_depth, depth_multiplier)
                    )
                    _, pointwise_val = parse_shape_or_val(
                        (1, 1) + (input_depth * depth_multiplier, 7)
                    )

                    z_tf = KTF.eval(
                        KTF.separable_conv2d(
                            x_tf,
                            KTF.variable(depthwise_val),
                            KTF.variable(pointwise_val),
                            data_format=data_format,
                        )
                    )
                    z_c = cntk_func_three_tensor(
                        "separable_conv2d",
                        input_shape,
                        depthwise_val,
                        pointwise_val,
    ...
    def test_depthwise_conv_2d(self, k):
        for data_format in ["channels_first", "channels_last"]:
            ...
            z = k.eval(
                k.depthwise_conv2d(
                    k.variable(x_val), k.variable(kernel_val), data_format=data_format
                )
            )

            for z_i in np.split(
                z, 6, axis=1 if data_format == "channels_first" else -1
            ):
    ...
    def test_pooling_invalid_use(self):
        for input_shape, pool_size in zip(
            [(5, 10, 12, 3), (5, 10, 12, 6, 3)], [(2, 2), (2, 2, 2)]
        ):
            for k in BACKENDS:
    ...
    def test_resize_volumes(self):
        ...
        for k in BACKENDS:
            with pytest.raises(ValueError):
                k.resize_volumes(
                    k.variable(xval), 2, 2, 2, data_format="channels_middle"
                )

    def test_temporal_padding(self):
        check_single_tensor_operation("temporal_padding", (4, 3, 3), BACKENDS)
        check_single_tensor_operation(
            "temporal_padding", (2, 3, 4), BACKENDS, padding=(1, 2)
        )
        ...
        for k in BACKENDS:
            with pytest.raises(ValueError):
                k.spatial_2d_padding(
                    k.variable(xval), padding=padding, data_format="channels_middle"
                )
    ...
    def test_ctc(self):
        ...
        assert_allclose(
            res[0, :] if K.backend() == "theano" else res[:, 0], ref, atol=1e-05
        )
    ...
    def test_ctc_decode_greedy(self):
        ...
        inputs = [
            np.vstack([input_prob_matrix_0[t, :], input_prob_matrix_1[t, :]])
        ...
        decode_pred_tf, log_prob_pred_tf = KTF.ctc_decode(
            inputs, input_length, greedy=True
        )
    ...
    def test_ctc_decode_beam_search(self):
        ...
        # batch_size length vector of negative log probabilities
        log_prob_truth = np.array(
            [0.584855, 0.389139], np.float32  # output beam 0  # output beam 1
        )[np.newaxis, :]
    ...
    def test_in_train_phase(self):
        for training in [True, False]:
            check_two_tensor_operation(
                "in_train_phase", (3, 3), (2, 2), [KTH, KTF], training=training
            )
