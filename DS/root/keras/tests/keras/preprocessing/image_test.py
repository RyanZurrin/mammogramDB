...
class TestImage(object):
    ...
    def test_directory_iterator(self, tmpdir):
        ...
        test_x1 = image.load_img(
            os.path.join(dir_seq.directory, dir_seq.filenames[1]), grayscale=False
        )
    ...
    def test_load_img(self, tmpdir):
        ...
        loaded_im_array = image.img_to_array(loaded_im)
        assert loaded_im_array.shape == (
            original_im_array.shape[0],
            original_im_array.shape[1],
            1,
        )
        ...
        loaded_im_nearest = image.load_img(
            filename, target_size=(25, 25), interpolation="nearest"
        )
        ...
        with pytest.raises(ValueError):
            loaded_im = image.load_img(
                filename, target_size=(25, 25), interpolation="unsupported"
            )
