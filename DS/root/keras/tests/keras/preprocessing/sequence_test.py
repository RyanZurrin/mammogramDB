...
def test_pad_sequences_vector():
    ...
    assert_allclose(
        b,
        [[[0, 0], [0, 0], [1, 1]], [[0, 0], [2, 1], [2, 2]], [[3, 1], [3, 2], [3, 3]]],
    )
    b = pad_sequences(a, maxlen=3, padding="post")
    assert_allclose(
        b,
        [[[1, 1], [0, 0], [0, 0]], [[2, 1], [2, 2], [0, 0]], [[3, 1], [3, 2], [3, 3]]],
    )
    ...
    assert_allclose(
        b,
        [[[1, 1], [1, 1], [1, 1]], [[1, 1], [2, 1], [2, 2]], [[3, 1], [3, 2], [3, 3]]],
    )


def test_make_sampling_table():
    a = make_sampling_table(3)
    assert_allclose(a, np.asarray([0.00315225, 0.00315225, 0.00547597]), rtol=0.1)
