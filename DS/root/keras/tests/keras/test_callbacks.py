...
def test_LearningRateScheduler():
    ...
    cbks = [callbacks.LearningRateScheduler(lambda x: 1.0 / (1.0 + x))]
    model.fit(
        X_train,
        y_train,
        batch_size=batch_size,
...
def test_ReduceLROnPlateau():
    ...
    cbks = [
        callbacks.ReduceLROnPlateau(
            monitor="val_loss", factor=0.1, epsilon=10, patience=1, cooldown=5
        )
    ]
    model.fit(
        X_train,
...
def test_TensorBoard_convnet(tmpdir):
    ...
    model = Sequential(
        [
            Conv2D(
                filters=8, kernel_size=3, activation="relu", input_shape=input_shape
            ),
