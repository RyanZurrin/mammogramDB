...
def wider2net_conv2d(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    """
    assert (
        teacher_w1.shape[0] == teacher_w2.shape[1]
    ), "successive layers from teacher model should have compatible shapes"
    assert (
        teacher_w1.shape[3] == teacher_b1.shape[0]
    ), "weight and bias from same layer should have compatible shapes"
    assert (
        new_width > teacher_w1.shape[3]
    ...
    if init == "random-pad":
        ...
        new_w2 = np.random.normal(
            0, 0.1, size=teacher_w2.shape[:2] + (n, teacher_w2.shape[3])
        )
    elif init == "net2wider":
...
def wider2net_fc(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    """
    assert (
        teacher_w1.shape[1] == teacher_w2.shape[0]
    ), "successive layers from teacher model should have compatible shapes"
    assert (
        teacher_w1.shape[1] == teacher_b1.shape[0]
    ), "weight and bias from same layer should have compatible shapes"
    assert (
        new_width > teacher_w1.shape[1]
...
def make_wider_student_model(
    teacher_model, x_train, y_train, x_test, y_test, init, epochs
):
    ...
    model.add(
        Conv2D(
            new_conv1_width, 3, input_shape=input_shape, padding="same", name="conv1"
        )
    )
    model.add(MaxPooling2D(2, name="pool1"))
    model.add(Conv2D(64, 3, padding="same", name="conv2"))
...
def make_deeper_student_model(
    ...
    if init == "net2deeper":
        ...
        model.add(
            Conv2D(64, 3, padding="same", name="conv2-deeper", weights=new_weights)
        )
    ...
    if init == "net2deeper":
        ...
        model.add(
            Dense(
                64, kernel_initializer="identity", activation="relu", name="fc1-deeper"
            )
        )
    elif init == "random-init":
...
def net2wider_experiment():
    ...
    make_wider_student_model(
        teacher_model, x_train, y_train, x_test, y_test, init="net2wider", epochs=epochs
    )
