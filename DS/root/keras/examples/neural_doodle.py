...
parser.add_argument(
    "--style-mask", type=str, help="path to semantic mask of style image"
)
parser.add_argument(
    "--target-mask", type=str, help="path to semantic mask of target image"
)
parser.add_argument(
    "--content-image", type=str, default=None, help="path to optional content image"
)
parser.add_argument(
    "--target-image-prefix", type=str, help="path prefix for generated results"
)
...
def load_mask_labels():
    ...
    if K.image_data_format() == "channels_first":
        mask_vecs = np.vstack(
            [style_mask_img.reshape((3, -1)).T, target_mask_img.reshape((3, -1)).T]
        )
    else:
        mask_vecs = np.vstack(
            [style_mask_img.reshape((-1, 3)), target_mask_img.reshape((-1, 3))]
        )
    ...
    style_mask = np.stack(
        [style_mask_label == r for r in range(num_labels)], axis=stack_axis
    )
    target_mask = np.stack(
        [target_mask_label == r for r in range(num_labels)], axis=stack_axis
    )
...
if K.image_data_format() == "channels_first":
    x = np.random.uniform(0, 255, (1, 3, img_nrows, img_ncols)) - 128.0
else:
    x = np.random.uniform(0, 255, (1, img_nrows, img_ncols, 3)) - 128.0
