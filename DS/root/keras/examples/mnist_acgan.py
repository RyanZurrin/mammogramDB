...
def build_generator(latent_size):
    ...
    cls = Flatten()(
        Embedding(num_classes, latent_size, embeddings_initializer="glorot_normal")(
            image_class
        )
    )
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        num_rows = 40
        noise = np.tile(
            np.random.uniform(-1, 1, (num_rows, latent_size)), (num_classes, 1)
        )

        sampled_labels = np.array([[i] * num_rows for i in range(num_classes)]).reshape(
            -1, 1
        )
        ...
        real_labels = y_train[
            (epoch - 1) * num_rows * num_classes : epoch * num_rows * num_classes
        ]
        ...
        img = (
            np.concatenate(
                [r.reshape(-1, 28) for r in np.split(img, 2 * num_classes + 1)], axis=-1
            )
            * 127.5
            + 127.5
        ).astype(np.uint8)
