...
class EvaluateInputTensor(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        results = self.val_model.evaluate(
            None, None, steps=int(self.num_steps), verbose=self.verbose
        )
        metrics_str = "\n"
...
def cnn_layers(x_train_input):
    ...
    x_train_out = layers.Dense(num_classes, activation="softmax", name="x_train_out")(x)
    return x_train_out
...
loss, acc = test_model.evaluate(
    x_test, keras.utils.to_categorical(y_test), batch_size=batch_size
)
