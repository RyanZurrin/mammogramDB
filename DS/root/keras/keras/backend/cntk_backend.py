...
_LEARNING_PHASE = C.constant(
    shape=(), dtype=np.float32, value=1.0, name="_keras_learning_phase"
)
...
def set_learning_phase(value):
    ...
    if value not in {0, 1}:
        raise ValueError(
            "CNTK Backend: Set learning phase "
...
def _convert_dtype_string(dtype):
    if dtype == np.float32:
        ...
        raise ValueError(
            "CNTK Backend: Unsupported dtype: %s. "
...
def variable(value, dtype=None, name=None, constraint=None):
    ...
    if isinstance(value, C.variables.Constant) or isinstance(
        value, C.variables.Parameter
    ):
        value = value.value
...
def placeholder(
    shape=None, ndim=None, dtype=None, sparse=False, name=None, dynamic_axis_num=1
):
    if dtype is None:
        dtype = floatx()
    if not shape:
    ...
    if dynamic_axis_num > len(cntk_shape):
        raise ValueError(
            "CNTK backend: creating placeholder with "
            "%d dimension is not supported, at least "
            "%d dimensions are needed." % (len(cntk_shape, dynamic_axis_num))
        )
...
def is_keras_tensor(x):
    if not isinstance(
        ...
        raise ValueError(
            "Unexpectedly found an instance of type `" + str(type(x)) + "`. "
...
def constant(value, dtype=None, shape=None, name=None):
    ...
    const = C.constant(np_value, dtype=dtype, name=_prepare_name(name, "constant"))
...
def random_binomial(shape, p=0.0, dtype=None, seed=None):
    ...
    for _ in shape:
        if _ is None:
            raise ValueError(
                "CNTK Backend: randomness op with "
...
def random_uniform_variable(shape, low, high, dtype=None, name=None, seed=None):
    ...
    scale = (high - low) / 2
    p = C.parameter(
        shape, init=C.initializer.uniform(scale, seed=seed), dtype=dtype, name=name
    )
...
def random_normal_variable(shape, mean, scale, dtype=None, name=None, seed=None):
    ...
    return C.parameter(
        shape=shape,
        init=C.initializer.normal(scale=scale, seed=seed),
        dtype=dtype,
        name=name,
    )
...
def truncated_normal(shape, mean=0.0, stddev=1.0, dtype=None, seed=None):
    ...
    return C.parameter(
        shape, init=C.initializer.truncated_normal(stddev, seed=seed), dtype=dtype
    )
...
def count_params(x):
    for _ in x.shape:
        if _ == C.InferredDimension or _ == C.FreeDimension:
            raise ValueError(
                "CNTK backend: `count_params` with dynamic "
...
def batch_dot(x, y, axes=None):
    ...
    if len(x_shape) == 2 and len(y_shape) == 2:
        ...
        while i > 0:
            ...
            i -= 1
        result = C.times(
            x, y, output_rank=(len(y.shape) - 1) if len(y.shape) > 1 else 1
        )
        if len(y_shape) == 2:
...
def any(x, axis=None, keepdims=False):
    ...
    any_matrix = C.element_select(
        reduce_result, ones_like(reduce_result), zeros_like(reduce_result)
    )
...
def classification_error(target, output, axis=-1):
    return C.ops.reduce_mean(
        C.equal(argmax(output, axis=-1), argmax(target, axis=-1)),
...
def moving_average_update(variable, value, momentum):
    return C.assign(variable, variable * momentum + value * (1.0 - momentum))
...
def normalize_batch_in_training(x, gamma, beta, reduction_axes, epsilon=1e-3):
    ...
    if sorted(reduction_axes) == list(range(ndim(x)))[:-1]:
        normalized = batch_normalization(x, mean, variant, beta, gamma, epsilon)
    else:
        # need broadcasting
        target_shape = []
        x_shape = int_shape(x)
...
def permute_dimensions(x, pattern):
    ...
    if (
        num_dynamic_axis > 0
        and pattern[:num_dynamic_axis] != current_layout[:num_dynamic_axis]
    ):
        raise ValueError(
...
def _static_rnn(
    ...
    if shape[1] is None:
        raise ValueError(
            "CNTK Backend: the input of static rnn "
...
def rnn(
    ...
    if dims < 3:
        raise ValueError(
            "CNTK Backend: the input of rnn has only rank %d "
            ...
            if isinstance(constants[j], list):
                ...
                while i < len(constants[j]):
                    if _get_dynamic_axis_num(constants[j][i]) == 1:
                        constants[j][i] = C.sequence.broadcast_as(
                            constants[j][i], inputs
                        )
                    i += 1
            else:
    ...
    with C.default_options(axis_offset=1):
        ...
        def _recurrence(x, states, m):
            ...
            if m is not None:
                new_states = [
                    C.element_select(m, n, s) for n, s in zip(new_states, past_values)
                ]
            n_s = []
...
def conv2d(
    ...
    if dilation_rate == (1, 1):
        ...
        x = C.convolution(
            kernel, x, strides=dilation_rate[0], auto_padding=[False, padding, padding]
        )
...
def separable_conv2d(
    ...
    depthwise_kernel = C.reshape(
        C.transpose(depthwise_kernel, (1, 0, 2, 3)),
    ...
    if dilation_rate == (1, 1):
        ...
        x = C.convolution(pointwise_kernel, x, strides=(1, 1, 1), auto_padding=[False])
    else:
        if dilation_rate[0] != dilation_rate[1]:
            raise ValueError(
                "CNTK Backend: non-square dilation_rate is " "not supported."
            )
        if strides != (1, 1):
...
def conv3d(
    ...
    x = C.convolution(
        kernel, x, strides, auto_padding=[False, padding, padding, padding]
    )
...
def dropout(x, level, noise_shape=None, seed=None):
    if level < 0.0 or level >= 1:
        raise ValueError(
            "CNTK Backend: Invalid dropout level %s, "
    ...
    def __init__(self, inputs, outputs, updates=[], **kwargs):
        ...
        if len(updates) > 0:
            ...
            if len(u_list) > 0:
                ...
                criterion = (
                    (outputs[0], outputs[1]) if len(outputs) > 1 else (outputs[0],)
                )
...
def _padding(x, pattern, axis):
    ...
    if b_any([dim < 0 for dim in base_shape]):
        raise ValueError(
            "CNTK Backend: padding input tensor with "
...
def spatial_2d_padding(x, padding=((1, 1), (1, 1)), data_format=None):
    ...
    if data_format == "channels_first":
        if num_dynamic_axis > 0:
            ...
            if hasattr(C, "pad"):
                x = C.pad(
                    x, pattern=[[0, 0], [0, 0], list(padding[0]), list(padding[1])]
                )
            else:
        ...
        if num_dynamic_axis > 0:
            ...
            if hasattr(C, "pad"):
                x = C.pad(
                    x, pattern=[[0, 0], list(padding[0]), list(padding[1]), [0, 0]]
                )
            else:
                x = _padding(x, padding[0], 1)
                x = _padding(x, padding[1], 2)
...
def get_value(x):
    if isinstance(x, C.variables.Parameter) or isinstance(x, C.variables.Constant):
        return x.value
    else:
        return eval(x)
...
def print_tensor(x, message=""):
    return C.user_function(
        LambdaFunc(x, when=lambda x: True, execute=lambda x: print(message))
    )
...
def switch(condition, then_expression, else_expression):
    ...
    if ndim_cond > ndim_expr:
        raise ValueError(
            "Rank of condition should be less"
...
def local_conv2d(inputs, kernel, kernel_size, strides, output_shape, data_format=None):
    ...
    output = reshape(output, (-1, filters, output_row, output_col))
...
class ReshapeBatch(C.ops.functions.UserFunction):
    ...
    def infer_outputs(self):
        ...
        return [
            C.output_variable(self.target_shape, self.inputs[0].dtype, [batch_axis])
        ]
    ...
    def backward(self, state, root_gradients):
        ...
        return C.cntk_py.Value(
            grad_array_view.as_shape((num_old_batch,) + self.from_shape)
        )
...
class ConvertToBatch(C.ops.functions.UserFunction):
    ...
    def infer_outputs(self):
        ...
        return [
            C.output_variable(
                self.inputs[0].shape[1:], self.inputs[0].dtype, [batch_axis]
            )
        ]
...
class ConvertToStatic(C.ops.functions.UserFunction):
    ...
    def infer_outputs(self):
        return [C.output_variable(self.target_shape, self.inputs[0].dtype, [])]
...
class LambdaFunc(C.ops.functions.UserFunction):
    def __init__(
        self, arg, when=lambda arg: True, execute=lambda arg: print(arg), name=""
    ):
        self.when = when
    ...
    def infer_outputs(self):
        return [
            C.output_variable(
                self.inputs[0].shape, self.inputs[0].dtype, self.inputs[0].dynamic_axes
            )
        ]
