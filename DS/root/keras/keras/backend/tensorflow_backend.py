...
def clear_session():
    ...
    reset_uids()
    _SESSION = None
    phase = tf.placeholder_with_default(False, shape=(), name="keras_learning_phase")
    _GRAPH_LEARNING_PHASES = {}
...
def is_keras_tensor(x):
    ...
    ```
    """
    if not isinstance(x, (tf.Tensor, tf_variables.Variable, tf.SparseTensor)):
        raise ValueError(
...
def random_uniform_variable(shape, low, high, dtype=None, name=None, seed=None):
    ...
    value = tf.random_uniform_initializer(low, high, dtype=tf_dtype, seed=seed)(shape)
...
class Function(object):
    ...
    def __init__(self, inputs, outputs, updates=None, name=None, **session_kwargs):
        ...
        if not isinstance(inputs, (list, tuple)):
            raise TypeError(
                "`inputs` to a TensorFlow backend function "
                "should be a list or tuple."
            )
        if not isinstance(outputs, (list, tuple)):
            raise TypeError(
                "`outputs` of a TensorFlow backend function "
                "should be a list or tuple."
            )
        if not isinstance(updates, (list, tuple)):
            raise TypeError(
                "`updates` in a TensorFlow backend function "
                "should be a list or tuple."
            )
...
def function(inputs, outputs, updates=None, **kwargs):
    ...
    if kwargs:
        for key in kwargs:
            if not (
                has_arg(tf.Session.run, key, True)
                or has_arg(Function.__init__, key, True)
            ):
                msg = (
    ...
    if unroll:
        ...
        if mask is not None:
            ...
            for inp, mask_t in zip(input_list, mask_list):
                ...
                for state, new_state in zip(states, new_states):
                    ...
                    tiled_mask_t = tf.tile(
                        mask_t, tf.stack([1, tf.shape(new_state)[1]])
                    )
        ...
        if mask is not None:
            ...
            mask_ta = tensor_array_ops.TensorArray(
                dtype=tf.bool, size=time_steps, tensor_array_name="mask_ta"
            )
            ...
            def _step(time, output_ta_t, *states):
                ...
                new_states = [
                    tf.where(tiled_mask_t, new_states[i], states[i])
...
def hard_sigmoid(x):
    ...
    x = (0.2 * x) + 0.5
    zero = _to_tensor(0.0, x.dtype.base_dtype)
    one = _to_tensor(1.0, x.dtype.base_dtype)
...
def dropout(x, level, noise_shape=None, seed=None):
    ...
    return tf.nn.dropout(x * 1.0, retain_prob, noise_shape, seed=seed)
...
def random_binomial(shape, p=0.0, dtype=None, seed=None):
    ...
    if seed is None:
        seed = np.random.randint(10e6)
    return tf.where(
        tf.random_uniform(shape, dtype=dtype, seed=seed) <= p,
...
def ctc_label_dense_to_sparse(labels, label_lengths):
    ...
    label_array = tf.reshape(
        tf.tile(tf.range(label_shape[1]), num_batches_tns), label_shape
    )
    label_ind = tf.boolean_mask(label_array, dense_mask)

    batch_array = tf.transpose(
        tf.reshape(
            tf.tile(tf.range(label_shape[0]), max_num_labels_tns),
...
def ctc_decode(y_pred, input_length, greedy=True, beam_width=100, top_paths=1):
    ...
    decoded_dense = [
        tf.sparse_to_dense(st.indices, st.dense_shape, st.values, default_value=-1)
        for st in decoded
    ]
