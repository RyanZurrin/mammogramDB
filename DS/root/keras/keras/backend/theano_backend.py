...
_LEARNING_PHASE = T.scalar(
    dtype="uint8", name="keras_learning_phase"
)  # 0 = test, 1 = train
...
def _assert_sparse_module():
    if not th_sparse_module:
        raise ImportError(
            "Failed to import theano.sparse\n"
...
def variable(value, dtype=None, name=None, constraint=None):
    ...
    if hasattr(value, "tocoo"):
        ...
        variable = theano.shared(
            value=value, name=_prepare_name(name, "variable"), strict=False
        )
...
def constant(value, dtype=None, shape=None, name=None):
    ...
    const = T.constant(np_value, dtype=dtype, name=_prepare_name(name, "constant"))
...
def is_keras_tensor(x):
    ...
    ```
    """
    if not isinstance(x, (T.TensorVariable, T.sharedvar.TensorSharedVariable)):
        raise ValueError(
...
def random_uniform_variable(shape, low, high, dtype=None, name=None):
    return variable(
        np.random.uniform(low=low, high=high, size=shape), dtype=dtype, name=name
    )


def random_normal_variable(shape, mean, scale, dtype=None, name=None):
    return variable(
        np.random.normal(loc=0.0, scale=scale, size=shape), dtype=dtype, name=name
    )
...
def moving_average_update(variable, value, momentum):
    return (variable, variable * momentum + value * (1.0 - momentum))
...
# T.nnet.bn.batch_normalization_train is deprecated
def _old_normalize_batch_in_training(x, gamma, beta, reduction_axes, epsilon=1e-3):
    """Computes mean and std for batch then apply batch_normalization on batch."""
    if gamma is None:
        gamma = ones_like(x)
    if beta is None:
...
def _old_batch_normalization(x, mean, var, beta, gamma, epsilon=1e-3):
    ...
    return T.nnet.bn.batch_normalization(
        x, gamma, beta, mean, sqrt(var + epsilon), mode="high_mem"
    )
...
def reverse(x, axes):
    ...
    if isinstance(axes, int):
        axes = [axes]
    slices = [
        slice(None, None, -1) if i in axes else slice(None, None, None)
...
def get_value(x):
    if not hasattr(x, "get_value"):
        raise TypeError(
            "`get_value` can only be called on a variable. "
...
def _postprocess_conv2d_output(
    conv_out, x, padding, kernel_shape, strides, data_format
):
    if padding == "same":
...
def _postprocess_conv3d_output(
    conv_out, x, padding, kernel_shape, strides, data_format
):
    if padding == "same":
        if kernel_shape[2] % 2 == 0:
...
def conv2d_transpose(
    ...
    if padding == "same" and kernel_shape[0] % 2 == 0:
        raise ValueError(
            "In `Conv2DTranspose`, with padding mode `same`, "
...
def depthwise_conv2d(
    ...
    depthwise_kernel_shape = _preprocess_conv2d_filter_shape(
    ...
    depthwise_kernel_shape = (input_depth * output_depth, 1) + depthwise_kernel_shape[
        2:
    ]
...
def conv3d(
    ...
    conv_out = _postprocess_conv3d_output(
        conv_out, x, padding, kernel_shape, strides, data_format
    )
    return conv_out
...
def conv3d_transpose(
    ...
    if padding == "same" and kernel_shape[0] % 2 == 0:
        raise ValueError(
            "In `Conv3DTranspose`, with padding mode `same`, "
            "even kernel sizes are not supported with Theano. "
            "You can set `kernel_size` to an odd number."
        )
...
def pool2d(
    ...
    if padding == "same":
        w_pad = (
            pool_size[0] - 2
            if pool_size[0] > 2 and pool_size[0] % 2 == 1
            else pool_size[0] - 1
        )
        h_pad = (
            pool_size[1] - 2
            if pool_size[1] > 2 and pool_size[1] % 2 == 1
            else pool_size[1] - 1
        )
        pad = (w_pad, h_pad)
    elif padding == "valid":
    ...
    if pool_mode == "max":
        pool_out = pool.pool_2d(
            x, ws=pool_size, stride=strides, ignore_border=True, pad=pad, mode="max"
        )
    elif pool_mode == "avg":
        pool_out = pool.pool_2d(
            x,
...
def pool3d(
    ...
    if pool_mode == "max":
        pool_out = pool.pool_3d(
            x, ws=pool_size, stride=strides, ignore_border=True, pad=padding, mode="max"
        )
    elif pool_mode == "avg":
        pool_out = pool.pool_3d(
            x,
...
def bias_add(x, bias, data_format=None):
    ...
    if ndim(bias) != 1 and ndim(bias) != ndim(x) - 1:
        raise ValueError(
            ...
            "expect to be 1 or %d dimensions" % (ndim(bias), ndim(x) - 1)
        )
...
def ctc_update_log_p(skip_idxs, zeros, active, log_p_curr, log_p_prev):
    ...
    active_next = T.cast(
        T.minimum(
            T.maximum(
                active + 1, T.max(T.concatenate([active_skip_idxs, [-1]])) + 2 + 1
            ),
            log_p_curr.shape[0],
        ),
        "int32",
    ...
    log_p_next = T.set_subtensor(
        zeros[:active_next], log_p_curr[:active_next] + updated_log_p_prev
    )
    return active_next, log_p_next


def ctc_path_probs(predict, Y, alpha=1e-4):
    smoothed_predict = (1 - alpha) * predict[:, Y] + alpha * np.float32(1.0) / Y.shape[
        0
    ]
    ...
    mask = (idxs < f_active.dimshuffle(0, "x")) & (idxs < b_active.dimshuffle(0, "x"))[
        ::-1, ::-1
    ]
...
def ctc_cost(predict, Y):
    ...
    total_log_prob = (
        T.log(T.sum(T.exp(log_probs - common_factor)[mask.nonzero()])) + common_factor
    )
...
def foldl(fn, elems, initializer=None, name=None):
    ...
    return theano.foldl(lambda x, acc: fn(acc, x), elems, initializer, name=name)[0]
...
def foldr(fn, elems, initializer=None, name=None):
    ...
    # We need to change the order of the arguments because theano accepts x as
    # first parameter and accumulator as second
    return theano.foldr(lambda x, acc: fn(acc, x), elems, initializer, name=name)[0]
    ...
    if data_format == "channels_first":
        ...
        for i in range(output_row):
            for j in range(output_col):
                ...
                x_flatten = reshape(
                    inputs[:, :, slice_row, slice_col], (1, -1, feature_dim)
                )
                output.append(dot(x_flatten, kernel[i * output_col + j, :, :]))
