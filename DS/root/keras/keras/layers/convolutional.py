...
class Conv2DTranspose(Conv2D):
    ...
    def call(self, inputs):
        ...
        out_height = conv_utils.deconv_length(height, stride_h, kernel_h, self.padding)
        out_width = conv_utils.deconv_length(width, stride_w, kernel_w, self.padding)
        if self.data_format == "channels_first":
...
class Conv3DTranspose(Conv3D):
    ...
    def build(self, input_shape):
        if len(input_shape) != 5:
            raise ValueError(
                "Inputs should have rank " + str(5) + "; Received input shape:",
                str(input_shape),
            )
        if self.data_format == "channels_first":
            channel_axis = 1
    ...
    def call(self, inputs):
        ...
        # Infer the dynamic output shape:
        out_depth = conv_utils.deconv_length(depth, stride_d, kernel_d, self.padding)
        out_height = conv_utils.deconv_length(height, stride_h, kernel_h, self.padding)
        out_width = conv_utils.deconv_length(width, stride_w, kernel_w, self.padding)
...
class _SeparableConv(_Conv):
    ...
    def build(self, input_shape):
        if len(input_shape) < self.rank + 2:
            raise ValueError(
                "Inputs to `SeparableConv" + str(self.rank) + "D` "
