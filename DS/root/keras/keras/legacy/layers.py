...
class Merge(Layer):
    ...
    def _arguments_validation(
        self, layers, mode, concat_axis, dot_axes, node_indices, tensor_indices
    ):
        ...
        if not isinstance(layers, (list, tuple)) or len(layers) < 2:
            raise TypeError(
                "A Merge should only be applied to a list of "
        ...
        if mode in {"sum", "mul", "ave", "cos", "max"}:
            ...
            if len(input_shapes_set) > 1:
                raise ValueError(
                    "Only layers of same output shape can "
                    "be merged using "
                    + mode
                    + " mode. "
        ...
        if mode in {"cos", "dot"}:
            ...
            if not isinstance(self.dot_axes, (list, tuple)):
                raise TypeError("Invalid type for dot_axes - " "should be a list.")
            if len(self.dot_axes) != 2:
                raise ValueError(
                    "Invalid format for dot_axes - " "should contain two elements."
                )
            if not isinstance(self.dot_axes[0], int) or not isinstance(
                self.dot_axes[1], int
            ):
                raise ValueError(
                    "Invalid format for dot_axes - " 'list elements should be "int".'
                )
            if shape1[self.dot_axes[0]] != shape2[self.dot_axes[1]]:
                raise ValueError(
                    "Dimension incompatibility using dot mode: "
    ...
    def call(self, inputs, mask=None):
        if not isinstance(inputs, list) or len(inputs) <= 1:
            raise TypeError(
                "Merge must be called on a list of tensors "
    ...
    def compute_output_shape(self, input_shape):
        ...
        if callable(self.mode):
            if callable(self._output_shape):
                ...
                raise ValueError(
                    "The Merge layer " + self.name + " has a callable `mode` argument, "
        ...
        if self.mode in ["sum", "mul", "ave", "max"]:
            ...
            for shape in input_shapes[1:]:
                if (
                    output_shape[self.concat_axis] is None
                    or shape[self.concat_axis] is None
                ):
...
def merge(
    ...
    """
    warnings.warn(
        "The `merge` function is deprecated "
        "and will be removed after 08/2017. "
        "Use instead layers from `keras.layers.merge`, "
        "e.g. `add`, `concatenate`, etc.",
        stacklevel=2,
...
class MaxoutDense(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        warnings.warn(
            "The `MaxoutDense` layer is deprecated "
...
class Highway(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        warnings.warn(
            "The `Highway` layer is deprecated " "and will be removed after 06/2017."
        )
        if "transform_bias" in kwargs:
            kwargs.pop("transform_bias")
            warnings.warn(
                "`transform_bias` argument is deprecated and " "has been removed."
            )
        self.init = initializers.get(init)
    ...
    def build(self, input_shape):
        ...
        self.W_carry = self.add_weight(
            (input_dim, input_dim), initializer=self.init, name="W_carry"
        )
        if self.bias:
            self.b = self.add_weight(
                (input_dim,),
            ...
            self.b_carry = self.add_weight(
                (input_dim,), initializer="one", name="b_carry"
            )
        else:
            self.b_carry = None
    ...
    def __call__(self, inputs, initial_state=None, **kwargs):
        ...
        for tensor in initial_state:
            if hasattr(tensor, "_keras_history") != is_keras_tensor:
                raise ValueError(
                    "The initial state of an RNN layer cannot be"
