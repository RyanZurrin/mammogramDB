...
generate_legacy_method_interface = functools.partial(
    generate_legacy_interface, object_type="method"
)


def raise_duplicate_arg_error(old_arg, new_arg):
    raise TypeError(
        "For the `" + new_arg + "` argument, "
...
legacy_dropout_support = generate_legacy_interface(
    allowed_positional_args=["rate", "noise_shape", "seed"], conversions=[("p", "rate")]
)
...
def embedding_kwargs_preprocessor(args, kwargs):
    ...
    if "dropout" in kwargs:
        kwargs.pop("dropout")
        warnings.warn(
            "The `dropout` argument is no longer support in `Embedding`. "
...
def recurrent_args_preprocessor(args, kwargs):
    ...
    if "forget_bias_init" in kwargs:
        if kwargs["forget_bias_init"] == "one":
            ...
            kwargs.pop("forget_bias_init")
            warnings.warn(
                "The `forget_bias_init` argument "
                "has been ignored. Use `unit_forget_bias=True` "
                "instead to initialize with ones.",
                stacklevel=3,
            )
    if "input_dim" in kwargs:
        ...
        warnings.warn(
            "The `input_dim` and `input_length` arguments "
...
legacy_pooling2d_support = generate_legacy_interface(
    allowed_positional_args=["pool_size", "strides", "padding"],
    conversions=[("border_mode", "padding"), ("dim_ordering", "data_format")],
    value_conversions={
        "dim_ordering": {"tf": "channels_last", "th": "channels_first", "default": None}
    },
)
...
legacy_upsampling1d_support = generate_legacy_interface(
    allowed_positional_args=["size"], conversions=[("length", "size")]
)
...
def conv3d_args_preprocessor(args, kwargs):
    ...
    if len(args) == 5:
        ...
        if (
            "kernel_dim1" in kwargs
            and "kernel_dim2" in kwargs
            and "kernel_dim3" in kwargs
        ):
            kernel_size = (
                kwargs.pop("kernel_dim1"),
                kwargs.pop("kernel_dim2"),
                kwargs.pop("kernel_dim3"),
            )
            args = [args[0], args[1], kernel_size]
...
def batchnorm_args_preprocessor(args, kwargs):
    ...
    if "mode" in kwargs:
        ...
        if value != 0:
            raise TypeError(
                "The `mode` argument of `BatchNormalization` "
...
def zeropadding2d_args_preprocessor(args, kwargs):
    ...
    if "padding" in kwargs and isinstance(kwargs["padding"], dict):
        if set(kwargs["padding"].keys()) <= {
            ...
            warnings.warn(
                "The `padding` argument in the Keras 2 API no longer"
                "accepts dict types. You can now input argument as: "
                "`padding=(top_pad, bottom_pad, left_pad, right_pad)`.",
                stacklevel=3,
            )
    elif len(args) == 2 and isinstance(args[1], dict):
        if set(args[1].keys()) <= {"top_pad", "bottom_pad", "left_pad", "right_pad"}:
...
legacy_spatialdropoutNd_support = generate_legacy_interface(
    ...
    conversions=[("p", "rate"), ("dim_ordering", "data_format")],
    value_conversions={
        "dim_ordering": {"tf": "channels_last", "th": "channels_first", "default": None}
    },
)
...
def generator_methods_args_preprocessor(args, kwargs):
    ...
    if keras1_args.intersection(kwargs.keys()):
        warnings.warn(
            "The semantics of the Keras 2 argument "
...
legacy_input_support = generate_legacy_interface(
    allowed_positional_args=None, conversions=[("input_dtype", "dtype")]
)
