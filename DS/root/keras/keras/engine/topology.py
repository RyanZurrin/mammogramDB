...
class Layer(object):
    ...
    def add_weight(
        ...
        if dtype is None:
            dtype = K.floatx()
        weight = K.variable(
            initializer(shape), dtype=dtype, name=name, constraint=constraint
        )
    ...
    def assert_input_compatibility(self, inputs):
        ...
        for x in inputs:
            try:
                ...
                raise ValueError(
                    "Layer " + self.name + " was called with "
                    "an input that isn't a symbolic tensor. "
                    "Received type: "
                    + str(type(x))
                    + ". Full input: "
                    + str(inputs)
                    + ". All inputs to the layer "
                    "should be tensors."
                )
        ...
        if len(inputs) != len(input_spec):
            raise ValueError(
                "Layer " + self.name + " expects " + str(len(input_spec)) + " inputs, "
                "but it received "
                + str(len(inputs))
            ...
            if spec.axes:
                ...
                if x_shape is not None:
                    for axis, value in spec.axes.items():
                        if value is not None and x_shape[int(axis)] not in {
                            value,
                            None,
                        }:
                            raise ValueError(
    ...
    def __call__(self, inputs, **kwargs):
        ...
        with K.name_scope(self.name):
            ...
            if (
                hasattr(self, "activity_regularizer")
    ...
    def _add_inbound_node(
        ...
        for i in range(len(output_tensors)):
            ...
            output_tensors[i]._uses_learning_phase = (
                getattr(output_tensors[i], "_uses_learning_phase", False) or uses_lp
            )
    ...
    def compute_output_shape(self, input_shape):
        ...
        if hasattr(self, "get_output_shape_for"):
            msg = (
                "Class `{}.{}` defines `get_output_shape_for` but does not override `compute_output_shape`. "
                + "If this is a Keras 1 layer, please implement `compute_output_shape` to support Keras 2."
            )
            warnings.warn(
                msg.format(type(self).__module__, type(self).__name__), stacklevel=2
            )
        return input_shape
    ...
    def get_input_mask_at(self, node_index):
        ...
        """
        return self._get_node_attribute_at_index(
            node_index, "input_masks", "input mask"
        )
    ...
    def get_output_mask_at(self, node_index):
        ...
        """
        return self._get_node_attribute_at_index(
            node_index, "output_masks", "output mask"
        )
    ...
    def input(self):
        ...
        if len(self._inbound_nodes) > 1:
            raise AttributeError(
                "Layer " + self.name + " has multiple inbound nodes, "
    ...
    def set_weights(self, weights):
        ...
        for pv, p, w in zip(param_values, params, weights):
            if pv.shape != w.shape:
                raise ValueError(
                    "Layer weight shape " + str(pv.shape) + " not compatible with "
...
class InputLayer(Layer):
    ...
    def __init__(
        ...
        if not batch_input_shape:
            if not input_shape:
                raise ValueError(
                    "An Input layer should be passed either "
        ...
        if input_tensor is None:
            ...
            input_tensor = K.placeholder(
                shape=batch_input_shape, dtype=dtype, sparse=self.sparse, name=self.name
            )
        else:
...
class Container(Layer):
    ...
    def __init__(self, inputs, outputs, name=None):
        ...
        if len(set(self.outputs)) != len(self.outputs):
            warnings.warn(
                "The list of outputs passed to the model "
                "is redundant. "
        ...
        for i, layer in enumerate(self.input_layers):
            ...
            if not isinstance(layer, InputLayer):
                raise TypeError(
                    ...
                    "from layer type `{}`.".format(inputs, i, layer.__class__.__name__)
                )
    ...
    def stateful(self):
        return any(
            [(hasattr(layer, "stateful") and layer.stateful) for layer in self.layers]
        )
        ...
        for layer in getattr(self, "input_layers", []):
            if layer.input_spec is None:
                ...
                if not isinstance(layer.input_spec, list):
                    raise TypeError(
                        "Layer " + layer.name + " has an input_spec attribute that "
                        ...
                        if len(computed_tensors) == 1:
                            shapes = _to_list(
                                layer.compute_output_shape(
                                    computed_tensors[0]._keras_shape
                                )
                            )
                            uses_learning_phase = computed_tensors[
                                0
    ...
    def load_weights(self, filepath, by_name=False, skip_mismatch=False, reshape=False):
        ...
        with h5py.File(filepath, mode="r") as f:
            ...
            if by_name:
                ...
                load_weights_from_hdf5_group(f, self.layers, reshape=reshape)
...
def preprocess_weights_for_loading(
    layer, weights, original_keras_version=None, original_backend=None, reshape=False
):
    ...
    if layer.__class__.__name__ in conv_layers:
        ...
        if reshape and layer_weights_shape != weights[0].shape:
            if weights[0].size != np.prod(layer_weights_shape):
                raise ValueError(
                    "Weights must be of equal size to "
                    + "apply a reshape operation. "
                    + "Layer "
                    + layer.name
                    + "'s weights have shape "
                    + str(layer_weights_shape)
                    + " and size "
...
def _need_convert_kernel(original_backend):
    ...
    if original_backend is None:
        ...
        return False
    uses_correlation = {"tensorflow": True, "theano": False, "cntk": True}
