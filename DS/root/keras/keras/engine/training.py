        ...
        if loss is losses.categorical_crossentropy:
            if y.shape[-1] == 1:
                raise ValueError(
                    ...
                    "```\n"
                    "from keras.helpers import to_categorical\n"
...
class Model(Container):
        ...
        **kwargs
        ...
        if target_tensors is not None:
            if isinstance(target_tensors, list):
                ...
                raise TypeError(
                    "Expected `target_tensors` to be " "a list or dict, but got:",
                    target_tensors,
                )
    ...
    def _standardize_user_data(
        ...
        for output_shape, loss_fn in zip(self._feed_output_shapes, self._feed_loss_fns):
            if loss_fn is losses.sparse_categorical_crossentropy:
                ...
                not hasattr(loss_fn, "__name__")
                or getattr(losses, loss_fn.__name__, None) is None
            ):
        ...
        sample_weights = _standardize_sample_weights(
            sample_weight, self._feed_output_names
        )
        class_weights = _standardize_class_weights(
            class_weight, self._feed_output_names
        )
        sample_weights = [
            _standardize_weights(ref, sw, cw, mode)
            for (ref, sw, cw, mode) in zip(
                y, sample_weights, class_weights, self._feed_sample_weight_modes
            )
        ]
        ...
        _check_loss_and_target_compatibility(
            y, self._feed_loss_fns, self._feed_output_shapes
        )
        ...
        **kwargs
        ...
        if validation_data:
            ...
            val_x, val_y, val_sample_weights = self._standardize_user_data(
                val_x, val_y, sample_weight=val_sample_weight, batch_size=batch_size
            )
            if self.uses_learning_phase and not isinstance(K.learning_phase(), int):
    ...
    def evaluate(
        self, x=None, y=None, batch_size=None, verbose=1, sample_weight=None, steps=None
    ):
    ...
    def predict(self, x, batch_size=None, verbose=0, steps=None):
        ...
        # Validate user data.
        x = _standardize_input_data(
            x, self._feed_input_names, self._feed_input_shapes, check_batch_axis=False
        )
        if self.stateful:
    ...
    def fit_generator(
        ...
        The use of `keras.helpers.Sequence` guarantees the ordering
        and guarantees the single use of every input per epoch when
        using `use_multiprocessing=True`.

        # Arguments
            generator: A generator or an instance of `Sequence`
                (`keras.helpers.Sequence`) object in order to avoid
            ...
            shuffle: Boolean. Whether to shuffle the training data
                ...
                Only used with instances of `Sequence` (`keras.helpers.Sequence`).
            initial_epoch: Integer.
                Epoch at which to start training
                (useful for resuming a previous training run).
        ...
        if not is_sequence and use_multiprocessing and workers > 1:
            warnings.warn(
                UserWarning(
                    ...
                    " Please consider using the`keras.helpers.Sequence"
                    " class."
                )
            )
        if steps_per_epoch is None:
            if is_sequence:
                ...
                raise ValueError(
                    ...
                    " generator based on the `keras.helpers.Sequence`"
                    " class. Please specify `steps_per_epoch` or use"
                    " the `keras.helpers.Sequence` class."
                )
        ...
        if (
            val_gen
            and not isinstance(validation_data, Sequence)
            and not validation_steps
        ):
            raise ValueError(
                "`validation_steps=None` is only valid for a"
                " generator based on the `keras.helpers.Sequence`"
        ...
        if verbose:
            _callbacks.append(
                cbks.ProgbarLogger(
                    count_mode="steps", stateful_metrics=self.stateful_metric_names
                )
            )
        _callbacks += (callbacks or []) + [self.history]
        callbacks = cbks.CallbackList(_callbacks)
                        ...
                        if val_gen:
                            ...
                            if val_alt_metric is None:
                                ...
                                val_outs = self.predict(
                                    val_x, batch_size=batch_size, verbose=0
                                )
    ...
    def evaluate_generator(
        ...
        # Arguments
            generator: Generator yielding tuples (inputs, targets)
                ...
                or an instance of Sequence (keras.helpers.Sequence)
        ...
        if steps is None:
            if is_sequence:
                ...
                raise ValueError(
                    ...
                    " based on the `keras.helpers.Sequence` class."
                    " Please specify `steps` or use the"
                    " `keras.helpers.Sequence` class."
                )
        enqueuer = None
