...
class Sequential(Model):
        ...
        if not self.outputs:
            ...
            if not isinstance(layer, (InputLayer, legacy_layers.Merge)):
                ...
                if isinstance(layer, (Model, Sequential)):
                    ...
                    if not layer.layers:
                        raise ValueError(
                            "Cannot add an empty model " "to a `Sequential` model."
                        )
    ...
    def predict_classes(self, x, batch_size=None, verbose=0, steps=None):
        ...
        """
        proba = self.predict(x, batch_size=batch_size, verbose=verbose, steps=steps)
        if proba.shape[-1] > 1:
    ...
    def fit_generator(
        ...
        # Arguments
            ...
            shuffle: Whether to shuffle the order of the batches at
                ...
                of `Sequence` (keras.helpers.Sequence).
            initial_epoch: Epoch at which to start training
                (useful for resuming a previous training run).
    ...
    def legacy_get_config(self):
        ...
        if isinstance(self.layers[0], legacy_layers.Merge):
            ...
            config.append(
                {
                    "class_name": self.layers[0].__class__.__name__,
...
def _clone_functional_model(model, input_tensors=None):
    ...
    if input_tensors is None:
        ...
        for original_input_layer, cloned_input_layer in zip(
            model.input_layers, input_layers
        ):
